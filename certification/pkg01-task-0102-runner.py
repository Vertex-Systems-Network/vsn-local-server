...
CANDIDATE='24ab1344185c0bf3d107aa766d4509a68959ebc8d22e213a160dfcaf622eced3'
SOURCE_MANIFEST_SHA256='9f938d7c923bcf2dd30b8959f1873b4d46fefcbd08c10cb03750fc111657c1c4'
...
def main():
    ...
    repo=os.getenv('GITHUB_REPOSITORY'); run_id=os.getenv('GITHUB_RUN_ID'); run_attempt=os.getenv('GITHUB_RUN_ATTEMPT'); commit=os.getenv('GITHUB_SHA')
    ...
    att={
      'schema_version':1,'product_version':PRODUCT_VERSION,'candidate_id':CANDIDATE,'pack_id':'pkg01-build-foundation-0102','recorded_at':now(),
      'host':{'system':system,'release':platform.release(),'machine':machine,'python':platform.python_version()},
      'source_manifest_sha256':SOURCE_MANIFEST_SHA256,'tools':{},
      'ci':{'github_actions':gha,'run_id':run_id,'run_attempt':run_attempt,'repository':repo,'commit_sha':commit}
    }
    run_url=f'https://github.com/{repo}/actions/runs/{run_id}' if gha and repo and run_id else 'local://pkg01-build-foundation-0102'
    d={
      'schema_version':2,'package_id':'PKG-01','task_id':'01.02','candidate_id':CANDIDATE,'product_version':PRODUCT_VERSION,
      'rust_toolchain':RUST_VERSION,'target':TARGET,'recorded_at':now(),'tools':vals,'runner_attestation':att,
      'provenance':{'run_url':run_url,'commit_sha':commit,'repository':repo,'github_actions':gha,'run_id':run_id,'run_attempt':run_attempt},
      'all_pass':bool(exact and host_ok and ci_ok)
    }
    out=Path(a.output); out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(d,indent=2,sort_keys=True)+'\n')
    side=Path(str(out)+'.sha256'); side.write_text(f'{sha256(out)}  {out.name}\n')
    print(json.dumps({'all_pass':d['all_pass'],'tools':vals,'host':att['host'],'ci':att['ci'],'output':str(out)},indent=2,sort_keys=True))
