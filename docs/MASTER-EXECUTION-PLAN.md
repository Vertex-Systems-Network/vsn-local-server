...
Canonical product acceptance is currently **PKG-03 — Windows Installer** at `17/25 = 68.00%`. Tasks `03.01`–`03.17` are canonically DONE with exact evidence recorded in `certification/pkg03-windows-installer-v1.json`.

The deterministic resume cursor is `03.18`. Dependency-ready tasks are `03.18`, `03.19`, and `03.22`; later tasks remain blocked by the frozen DAG until their prerequisites are canonically DONE.
...
<!-- Canonical active-package machine state: PKG-03 17/25 IN_PROGRESS; READY 03.18,03.19,03.22; deterministic cursor 03.18; query live main SHA at execution time -->