...
Canonical product acceptance is currently **PKG-03 — Windows Installer** at `19/25 = 76.00%`. Tasks `03.01`–`03.19` are canonically DONE with exact evidence recorded in `certification/pkg03-windows-installer-v1.json`.

The deterministic resume cursor is `03.20`. Dependency-ready tasks are `03.20` and `03.22`; later tasks remain blocked by the frozen DAG until their prerequisites are canonically DONE.
