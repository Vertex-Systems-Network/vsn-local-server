...
Canonical product acceptance is currently **PKG-03 — Windows Installer** at `15/25 = 60.00%`. Tasks `03.01`–`03.15` are canonically DONE with exact evidence recorded in `certification/pkg03-windows-installer-v1.json`.

The deterministic resume cursor is `03.16`. Dependency-ready tasks are `03.16`, `03.17`, `03.18`, `03.19`, and `03.22`; later tasks remain blocked by the frozen DAG until their prerequisites are canonically DONE.
