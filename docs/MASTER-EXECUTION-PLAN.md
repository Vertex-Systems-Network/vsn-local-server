...
Canonical product acceptance is currently **PKG-03 — Windows Installer** at `14/25 = 56.00%`. Tasks `03.01`–`03.14` are canonically DONE with exact evidence recorded in `certification/pkg03-windows-installer-v1.json`.

The deterministic resume cursor is `03.15`. Dependency-ready tasks are `03.15`, `03.17`, and `03.22`; dependent Wave 4+ tasks remain blocked by the frozen DAG until their prerequisites are canonically DONE.
...
<!-- Canonical active-package machine state: PKG-03 14/25 IN_PROGRESS; READY 03.15,03.17,03.22; deterministic cursor 03.15; query live main SHA at execution time -->
