# VSN Dev Platform 0.33 — P30 Runner Attestation & Portable Certification
...
## 0.33 P30 handoff/resume sprint
- Added source-manifest-driven `p30-handoff.py` portable pack bundles.
- Added `p30-gh-dispatch.py` for workflow dispatch, watch, artifact download, and candidate-bound merge.
- Added `--resume-ledger` to `p30-run-pack.py`; previously-valid PASS controls are protected from downgrade during a recovery rerun.
- Added P30 handoff schema and regression tests.
- Certification percentage remains evidence-driven; packaging/dispatch improvements do not count as PASS evidence.
