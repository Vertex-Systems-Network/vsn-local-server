# P30 stable-release readiness — VSN 0.18
...
`docs/release-evidence-0.18.json` begins with every item `pending`. `python scripts/release-evidence.py evaluate --file ... --require-certified` exits non-zero until every required item is `pass` or an explicitly reviewed `waived` control.
