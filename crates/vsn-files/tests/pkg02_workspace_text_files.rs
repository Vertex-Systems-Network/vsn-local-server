...
fn recursive_delete_removes_link_entry_not_target() {
    ...
    assert!(
        ...
        "link entry survived delete"
    );
    assert!(
        target.join("keep.txt").is_file(),
        "delete followed link target"
    );
...
fn move_renames_link_entry_not_target() {
    ...
    assert!(
        fs::symlink_metadata(&link).is_err(),
        "old link entry survived move"
    );
    assert!(
        fs::symlink_metadata(&moved).is_ok(),
        "moved link entry is missing"
    );
    assert!(
        target.join("keep.txt").is_file(),
        "move renamed link target"
    );
...
fn write_through_parent_link_outside_workspace_is_rejected() {
    ...
    assert_eq!(
        fs::read(outside.join("keep.txt")).expect("read sentinel"),
        b"outside"
    );
