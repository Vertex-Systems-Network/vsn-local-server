...
use vsn_database::{
    BrowsePage, BrowseRequest, CapabilitySet, DataModel, DatabaseError, DatabaseProvider,
    EntityMeta, EntityStatistics, FieldMeta, FieldType, IndexMeta, MutationRequest, MutationResult,
    RelationMeta,
};
...
impl SqliteProvider {
    ...
    pub fn open(path: &Path, read_only: bool) -> Result<Self, DatabaseError> {
        let mut provider = Self::new(read_only);
        provider.connect(&json!({"path":path}))?;
        Ok(provider)
    }
    ...
    fn conn(&self) -> Result<MutexGuard<'_, Connection>, DatabaseError> {
        self.connection
            .as_ref()
            .ok_or_else(|| DatabaseError::Provider("SQLite provider is not connected".into()))?
            .lock()
            .map_err(|_| DatabaseError::Provider("SQLite connection lock poisoned".into()))
    }
    fn ensure_write(&self) -> Result<(), DatabaseError> {
        if self.read_only {
            Err(DatabaseError::Invalid(
                "SQLite provider is read-only".into(),
            ))
        } else {
            Ok(())
        }
...
impl DatabaseProvider for SqliteProvider {
    ...
    fn connect(&mut self, connection: &Value) -> Result<(), DatabaseError> {
        let raw = connection
            ...
            .ok_or_else(|| DatabaseError::Invalid("SQLite connection requires path".into()))?;
    ...
    fn list_entities(&self, _namespace: Option<&str>) -> Result<Vec<String>, DatabaseError> {
        let conn = self.conn()?;
        let mut stmt=conn.prepare("SELECT name FROM sqlite_schema WHERE type IN ('table','view') AND name NOT LIKE 'sqlite_%' ORDER BY name").map_err(db_err)?;
        let rows = stmt
    ...
    fn describe_entity(
        ...
        let conn = self.conn()?;
        let sql = format!("PRAGMA table_info({})", quote_identifier(entity));
        ...
        let mut fields = rows.collect::<Result<Vec<_>, _>>().map_err(db_err)?;
        attach_foreign_keys(&conn, entity, &mut fields)?;
        let kind: String = conn
            .query_row(
                "SELECT type FROM sqlite_schema WHERE name=?1 LIMIT 1",
                [entity],
                |row| row.get(0),
            )
            .map_err(db_err)?;
        Ok(EntityMeta {
            ...
            fields,
            metadata: json!({"engine":"sqlite","kind":kind}),
        })
    }
    fn query(&self, statement: &str, parameters: &Value) -> Result<Value, DatabaseError> {
        validate_read_query(statement)?;
        let bind = json_params(parameters)?;
        let conn = self.conn()?;
        let mut stmt = conn.prepare(statement).map_err(db_err)?;
        let names: Vec<String> = stmt
            ...
            .collect();
        if names.is_empty() {
            return Err(DatabaseError::Invalid("query returned no columns".into()));
        }
        ...
        while let Some(row) = rows.next().map_err(db_err)? {
            if count >= 10_000 {
                return Err(DatabaseError::Invalid(
                    "query row limit exceeded (10000)".into(),
                ));
            }
    ...
    fn browse(
        ...
        let offset = request.offset.min(10_000_000_000);
        let meta = self.describe_entity(None, entity)?;
        let order = if let Some(field) = request.order_by.as_ref() {
            validate_identifier(field)?;
            if !meta.fields.iter().any(|f| f.name == *field) {
                return Err(DatabaseError::Invalid(
                    "order_by is not a field on the entity".into(),
                ));
            }
            format!(
                " ORDER BY {} {}",
        ...
        let total_i64: i64 = conn
            .query_row(
                &format!("SELECT COUNT(*) FROM {}", quote_identifier(entity)),
                [],
                |row| row.get(0),
            )
            .map_err(db_err)?;
    ...
    fn insert(
        ...
        validate_map(&request.values, 64)?;
        let fields = request.values.keys().cloned().collect::<Vec<_>>();
        ...
        let values = fields
            .iter()
            .map(|f| json_to_sql(request.values.get(f).expect("known key")))
    ...
    fn update(
        ...
        validate_identifier(entity)?;
        if request.values.is_empty() {
            return Err(DatabaseError::Invalid("update requires values".into()));
        }
        if request.filter.is_empty() {
            return Err(DatabaseError::Invalid(
                "update requires a non-empty equality filter".into(),
            ));
        }
        validate_map(&request.values, 64)?;
        validate_map(&request.filter, 32)?;
        let value_fields = request.values.keys().cloned().collect::<Vec<_>>();
        let filter_fields = request.filter.keys().cloned().collect::<Vec<_>>();
        let sets = value_fields
            .iter()
            .enumerate()
            .map(|(i, f)| format!("{}=?{}", quote_identifier(f), i + 1))
            .collect::<Vec<_>>()
            .join(",");
        let where_sql = filter_fields
            .iter()
            .enumerate()
            .map(|(i, f)| format!("{}=?{}", quote_identifier(f), value_fields.len() + i + 1))
            .collect::<Vec<_>>()
            .join(" AND ");
        let sql = format!(
            "UPDATE {} SET {} WHERE {}",
        ...
        values.extend(
            filter_fields
                .iter()
                .map(|f| json_to_sql(request.filter.get(f).expect("known key")))
                .collect::<Result<Vec<_>, _>>()?,
        );
        let conn = self.conn()?;
        let affected = conn
    ...
    fn delete(
        ...
        if request.filter.is_empty() {
            return Err(DatabaseError::Invalid(
                "delete requires a non-empty equality filter".into(),
            ));
        }
        validate_map(&request.filter, 32)?;
        let fields = request.filter.keys().cloned().collect::<Vec<_>>();
        let where_sql = fields
            .iter()
            .enumerate()
            .map(|(i, f)| format!("{}=?{}", quote_identifier(f), i + 1))
            .collect::<Vec<_>>()
    ...
    fn list_indexes(
        ...
        let conn = self.conn()?;
        let mut stmt = conn
            .prepare(&format!("PRAGMA index_list({})", quote_identifier(entity)))
            .map_err(db_err)?;
        let base = stmt
            .query_map([], |row| {
                Ok((
        ...
        drop(stmt);
        let mut out = Vec::new();
        for (name, unique, origin) in base {
    ...
    fn statistics(
        ...
        Ok(EntityStatistics {
            entity: entity.into(),
            row_count: Some(u64::try_from(count.max(0)).unwrap_or(0)),
            storage_bytes: file_bytes,
            index_bytes: None,
            metadata: json!({"database_file_bytes":file_bytes}),
        })
    }
}

pub fn inspect(path: &Path) -> Result<Value, DatabaseError> {
    let provider = SqliteProvider::open(path, true)?;
    let entities = provider.list_entities(None)?;
    let mut described = Vec::new();
    for entity in entities {
        let meta = provider.describe_entity(None, &entity)?;
        let indexes = provider.list_indexes(None, &entity).unwrap_or_default();
        let relations = provider.list_relations(None, &entity).unwrap_or_default();
        let statistics = provider.statistics(None, &entity).ok();
        described.push(
            json!({"entity":meta,"indexes":indexes,"relations":relations,"statistics":statistics}),
        );
    }
    Ok(
        json!({"provider":"sqlite","path":path,"capabilities":provider.capabilities(),"entities":described}),
    )
}
...
fn attach_foreign_keys(
    ...
    for rel in relations_for(conn, entity)? {
        for field in rel.from_fields {
            if let Some(item) = fields.iter_mut().find(|f| f.name == field) {
                item.field_type = FieldType::Relation;
                item.relation_target = Some(rel.to_entity.clone());
            }
        }
    }
    Ok(())
}
fn relations_for(conn: &Connection, entity: &str) -> Result<Vec<RelationMeta>, DatabaseError> {
    let sql = format!("PRAGMA foreign_key_list({})", quote_identifier(entity));
    let mut stmt = conn.prepare(&sql).map_err(db_err)?;
    let rows = stmt
        .query_map([], |row| {
            Ok((
        ...
        .map_err(db_err)?;
    let mut grouped: HashMap<i64, RelationMeta> = HashMap::new();
    for row in rows {
        let (id, target, from, to, on_update, on_delete) = row.map_err(db_err)?;
        let rel = grouped.entry(id).or_insert_with(|| RelationMeta {
            name: format!("fk_{entity}_{id}"),
            from_entity: entity.into(),
            from_fields: vec![],
        ...
        rel.from_fields.push(from);
        rel.to_fields.push(to);
    }
    let mut out = grouped.into_values().collect::<Vec<_>>();
    out.sort_by(|a, b| a.name.cmp(&b.name));
    Ok(out)
}
fn sqlite_type(value: &str) -> FieldType {
    let v = value.to_ascii_uppercase();
    if v.contains("INT") {
        FieldType::Integer
    } else if v.contains("CHAR") || v.contains("CLOB") || v.contains("TEXT") {
        FieldType::Text
    } else if v.contains("BLOB") {
        FieldType::Binary
    } else if v.contains("REAL")
        || v.contains("FLOA")
        || v.contains("DOUB")
        || v.contains("NUM")
        || v.contains("DEC")
    {
        ...
        FieldType::Boolean
    } else if v.contains("DATE") && v.contains("TIME") {
        FieldType::DateTime
    } else if v.contains("DATE") {
        FieldType::Date
    } else if v.contains("JSON") {
        FieldType::Json
    } else {
        FieldType::Unknown
    }
}
fn value_ref(v: ValueRef<'_>) -> Value {
    match v {
        ...
        ValueRef::Text(v) => {
            if v.len() > MAX_TEXT_CELL_BYTES {
                json!({"type":"text","bytes":v.len(),"truncated":true})
            } else {
    ...
    Ok(match v {
        ...
        Value::Number(n) => {
            if let Some(i) = n.as_i64() {
                ...
                return Err(DatabaseError::Invalid(
                    "numeric value is outside SQLite range".into(),
                ));
            }
        }
        Value::String(v) => {
            if v.len() > 8 * 1024 * 1024 {
                return Err(DatabaseError::Invalid("string value exceeds 8 MiB".into()));
            }
            SqlValue::Text(v.clone())
        }
        Value::Array(_) | Value::Object(_) => {
            let text =
                serde_json::to_string(v).map_err(|e| DatabaseError::Invalid(e.to_string()))?;
            if text.len() > 8 * 1024 * 1024 {
                return Err(DatabaseError::Invalid("JSON value exceeds 8 MiB".into()));
            }
            SqlValue::Text(text)
        }
    })
}
fn json_params(v: &Value) -> Result<Vec<SqlValue>, DatabaseError> {
    match v {
        Value::Null => Ok(vec![]),
        Value::Array(items) => {
            if items.len() > 1024 {
                return Err(DatabaseError::Invalid(
                    "too many SQLite bind parameters".into(),
                ));
            }
            items.iter().map(json_to_sql).collect()
        }
        _ => Err(DatabaseError::Invalid(
            "SQLite parameters must be a JSON array or null".into(),
        )),
    }
}
fn validate_map(map: &BTreeMap<String, Value>, max: usize) -> Result<(), DatabaseError> {
    if map.len() > max {
        return Err(DatabaseError::Invalid("too many mutation fields".into()));
    }
    for key in map.keys() {
...
fn quote_identifier(v: &str) -> String {
    format!("\"{}\"", v.replace('"', "\"\""))
}
fn validate_identifier(v: &str) -> Result<(), DatabaseError> {
    if v.is_empty() || v.len() > 255 || v.contains('\0') {
        Err(DatabaseError::Invalid("unsafe SQLite identifier".into()))
    } else {
        Ok(())
    }
}
fn validate_read_query(v: &str) -> Result<(), DatabaseError> {
    let trimmed = v.trim();
    if trimmed.is_empty() || trimmed.len() > 1024 * 1024 {
        return Err(DatabaseError::Invalid("query is empty or too large".into()));
    }
    let no_trailing = trimmed.trim_end_matches(';').trim_end();
    if no_trailing.contains(';') {
        return Err(DatabaseError::Invalid(
            "multiple SQL statements are not allowed".into(),
        ));
    }
    let upper = no_trailing.trim_start().to_ascii_uppercase();
    if upper.starts_with("SELECT ") || upper == "SELECT" {
        return Ok(());
    }
    if upper.starts_with("EXPLAIN SELECT ") || upper.starts_with("EXPLAIN QUERY PLAN SELECT ") {
        return Ok(());
    }
    if upper.starts_with("PRAGMA ") {
        ...
        if risky.iter().any(|x| upper.contains(x)) {
            return Err(DatabaseError::Invalid(
                "mutating/operational PRAGMA is not allowed through read query".into(),
            ));
        }
        return Ok(());
    }
    Err(DatabaseError::Invalid(
        "read query accepts SELECT, EXPLAIN SELECT, or non-mutating PRAGMA only".into(),
    ))
}
fn db_err<E: std::fmt::Display>(e: E) -> DatabaseError {
    DatabaseError::Provider(e.to_string())
}
...
mod tests {
    ...
    fn read_only_gate_blocks_mutation_and_with() {
        ...
        assert!(validate_read_query("WITH x AS (SELECT 1) SELECT * FROM x").is_err());
        assert!(validate_read_query("SELECT * FROM users").is_ok());
    }
    #[test]
    fn sqlite_affinity_mapping() {
        assert_eq!(sqlite_type("INTEGER"), FieldType::Integer);
        assert_eq!(sqlite_type("TEXT"), FieldType::Text);
    }
    #[test]
    ...
    fn crud_browse_indexes_relations_and_stats_roundtrip() {
        ...
        insert_values.insert("name".into(), json!("Ada"));
        insert_values.insert("team_id".into(), json!(1));
        ...
        assert!(indexes
            .iter()
            .any(|i| i.name == "idx_users_name" && i.unique));
        let relations = provider.list_relations(None, "users").unwrap();
        assert!(relations
            .iter()
            .any(|r| r.to_entity == "teams" && r.from_fields == vec!["team_id"]));
        let stats = provider.statistics(None, "users").unwrap();
        ...
        let page = provider
            ...
            .unwrap();
        assert_eq!(page.rows[0].get("name"), Some(&json!("Ada Lovelace")));
        provider
            .delete(
                None,
                "users",
            ...
            .unwrap();
        assert_eq!(
            provider.statistics(None, "users").unwrap().row_count,
            Some(0)
        );
        drop(provider);
        let _ = std::fs::remove_file(path);
