...
struct UnsignedAgentStreamHello<'a> {
    version: u32,
    device_id: &'a str,
    public_key: &'a str,
...
impl AgentStreamHelloV1 {
    fn canonical_bytes(&self) -> Result<Vec<u8>, RemoteError> {
        Ok(serde_json::to_vec(&UnsignedAgentStreamHello {
...
pub fn build_agent_stream_hello(
    identity: &DeviceIdentity,
) -> Result<AgentStreamHelloV1, RemoteError> {
    let meta = identity.metadata();
    let mut hello = AgentStreamHelloV1 {
        version: STREAM_RELAY_PROTOCOL_VERSION,
        device_id: meta.device_id.clone(),
        public_key: meta.public_key.clone(),
        timestamp_unix_ms: now_ms(),
        nonce: random_id("streamhello"),
        signature: String::new(),
    };
    hello.signature = identity.sign(&hello.canonical_bytes()?);
    Ok(hello)
}
...
pub fn verify_agent_stream_hello(hello: &AgentStreamHelloV1) -> Result<(), RemoteError> {
    ...
    if !safe_id(&hello.device_id, 8, 160) || !safe_id(&hello.nonce, 16, 160) {
        return Err(RemoteError::InvalidId);
    }
    validate_fresh_timestamp(hello.timestamp_unix_ms)?;
    DeviceIdentity::verify_with_public_key(
        &hello.public_key,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct RelayStreamFrameV1 {
    pub seq: u64,
    pub eof: bool,
    pub payload_base64: String,
}
impl RelayStreamFrameV1 {
    pub fn decoded_len(&self) -> Result<usize, RemoteError> {
        let bytes = B64
            .decode(&self.payload_base64)
            .map_err(|_| RemoteError::Transport("relay frame payload is not base64".into()))?;
        if bytes.len() > MAX_STREAM_RELAY_FRAME_BYTES {
            return Err(RemoteError::Transport("relay frame exceeds 256 KiB".into()));
        }
        Ok(bytes.len())
    }
}
...
pub enum BrowserStreamClientMessageV1 {
    Hello {
        ...
        request: RelayStreamOpenV1,
        #[serde(default)]
        resume: Option<BrowserStreamResumeV1>,
    },
    Input {
        frame: RelayStreamFrameV1,
    },
...
pub fn build_device_enrollment(
    identity: &DeviceIdentity,
    pairing_nonce: &str,
) -> Result<DeviceEnrollmentV1, RemoteError> {
    if !safe_id(pairing_nonce, 32, 256) {
        return Err(RemoteError::InvalidId);
    }
    let meta = identity.metadata();
...
pub fn verify_device_enrollment(enrollment: &DeviceEnrollmentV1) -> Result<(), RemoteError> {
    if enrollment.version != REMOTE_PROTOCOL_VERSION {
        return Err(RemoteError::Version);
    }
    if !safe_id(&enrollment.pairing_nonce, 32, 256) {
        return Err(RemoteError::InvalidId);
    }
    let derived_device_id = vsn_security::device_id_from_public_key_b64(&enrollment.public_key)?;
    if enrollment.device_id != derived_device_id {
        return Err(RemoteError::DeviceMismatch);
    }
    let proof = enrollment_proof_bytes(
        &enrollment.device_id,
        &enrollment.public_key,
        &enrollment.pairing_nonce,
    )?;
...
pub fn build_agent_poll(identity: &DeviceIdentity) -> Result<AgentPollV1, RemoteError> {
    let meta = identity.metadata();
    let mut poll = AgentPollV1 {
        version: REMOTE_PROTOCOL_VERSION,
        device_id: meta.device_id.clone(),
        public_key: meta.public_key.clone(),
        timestamp_unix_ms: now_ms(),
        nonce: random_id("poll"),
        signature: String::new(),
    };
    poll.signature = identity.sign(&poll.canonical_bytes()?);
    Ok(poll)
}

pub fn verify_agent_poll(poll: &AgentPollV1) -> Result<(), RemoteError> {
    if poll.version != REMOTE_PROTOCOL_VERSION {
        return Err(RemoteError::Version);
    }
    if !safe_id(&poll.nonce, 16, 128) {
        return Err(RemoteError::InvalidId);
    }
    let derived = vsn_security::device_id_from_public_key_b64(&poll.public_key)?;
    if derived != poll.device_id {
        return Err(RemoteError::DeviceMismatch);
    }
    validate_fresh_timestamp(poll.timestamp_unix_ms)?;
    vsn_security::verify_signature(&poll.public_key, &poll.canonical_bytes()?, &poll.signature)?;
    Ok(())
}
...
) -> Result<AgentCommandResultV1, RemoteError> {
    let mut result = AgentCommandResultV1 {
        version: REMOTE_PROTOCOL_VERSION,
        ...
        payload,
        timestamp_unix_ms: now_ms(),
        nonce: random_id("result"),
        signature: String::new(),
    };
    result.signature = identity.sign(&result.canonical_bytes()?);
    Ok(result)
}

pub fn verify_agent_result(
    result: &AgentCommandResultV1,
    public_key: &str,
) -> Result<(), RemoteError> {
    if result.version != REMOTE_PROTOCOL_VERSION {
        return Err(RemoteError::Version);
    }
    if !safe_id(&result.command_id, 16, 128)
        || !safe_id(&result.session_id, 16, 256)
        || !safe_id(&result.nonce, 16, 128)
    {
        return Err(RemoteError::InvalidId);
    }
    validate_fresh_timestamp(result.timestamp_unix_ms)?;
    vsn_security::verify_signature(public_key, &result.canonical_bytes()?, &result.signature)?;
    Ok(())
}
...
impl RemoteCommandVerifier {
    pub fn new(
        control_plane_public_key: impl Into<String>,
        expected_device_id: impl Into<String>,
    ) -> Self {
        Self {
            control_plane_public_key: control_plane_public_key.into(),
            expected_device_id: expected_device_id.into(),
            replay: Mutex::new(ReplayCache::new(4096)),
        }
    }
    ...
    pub fn verify_delivery(
        &self,
        command: &RemoteCommandV1,
    ) -> Result<DeliveryVerification, RemoteError> {
        if command.version != REMOTE_PROTOCOL_VERSION {
            return Err(RemoteError::Version);
        }
        if command.device_id != self.expected_device_id {
            return Err(RemoteError::DeviceMismatch);
        }
        if !safe_id(&command.command_id, 16, 128)
            || !safe_id(&command.session_id, 16, 256)
            || command.principal_id.is_empty()
            || command.permission.is_empty()
            || command.command.is_empty()
        {
            return Err(RemoteError::InvalidId);
        }
        if command.expires_at_unix_ms < command.issued_at_unix_ms
            || command.expires_at_unix_ms - command.issued_at_unix_ms > MAX_REMOTE_COMMAND_TTL_MS
        {
            return Err(RemoteError::Ttl);
        }
        let now = now_ms();
        if command.issued_at_unix_ms > now.saturating_add(MAX_CLOCK_SKEW_MS)
            || command.expires_at_unix_ms.saturating_add(MAX_CLOCK_SKEW_MS) < now
        {
            return Err(RemoteError::Timing);
        }
        let canonical = command.canonical_bytes()?;
        vsn_security::verify_signature(
            &self.control_plane_public_key,
            &canonical,
            &command.signature,
        )?;
        let mut replay = self.replay.lock().map_err(|_| RemoteError::Replay)?;
        if replay.insert(command.command_id.clone()) {
            Ok(DeliveryVerification::Fresh)
        } else {
            Ok(DeliveryVerification::Duplicate)
        }
    }

    pub fn verify(&self, command: &RemoteCommandV1) -> Result<(), RemoteError> {
        match self.verify_delivery(command)? {
            DeliveryVerification::Fresh => Ok(()),
            DeliveryVerification::Duplicate => Err(RemoteError::Replay),
        }
    }
}
...
impl AgentPollV1 {
    pub fn canonical_bytes(&self) -> Result<Vec<u8>, RemoteError> {
        Ok(serde_json::to_vec(&UnsignedAgentPoll {
            version: self.version,
            device_id: &self.device_id,
            public_key: &self.public_key,
            timestamp_unix_ms: self.timestamp_unix_ms,
...
impl AgentCommandResultV1 {
    pub fn canonical_bytes(&self) -> Result<Vec<u8>, RemoteError> {
        Ok(serde_json::to_vec(&UnsignedAgentResult {
            version: self.version,
            device_id: &self.device_id,
            command_id: &self.command_id,
            session_id: &self.session_id,
...
struct AgentResultCacheFile {
    entries: Vec<AgentCommandResultV1>,
}

fn load_agent_result_cache(path: &Path) -> Result<AgentResultCacheFile, RemoteError> {
    if !path.exists() {
        return Ok(AgentResultCacheFile::default());
    }
...
fn save_agent_result_cache(path: &Path, cache: &AgentResultCacheFile) -> Result<(), RemoteError> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(|e| RemoteError::Transport(e.to_string()))?;
    }
    ...
    if path.exists() {
        fs::remove_file(path).map_err(|e| RemoteError::Transport(e.to_string()))?;
    }
    fs::rename(tmp, path).map_err(|e| RemoteError::Transport(e.to_string()))?;
    Ok(())
}
...
pub fn load_cached_agent_result(
    path: &Path,
    command_id: &str,
) -> Result<Option<AgentCommandResultV1>, RemoteError> {
    Ok(load_agent_result_cache(path)?
        .entries
        .into_iter()
        .rev()
...
pub fn store_cached_agent_result(
    path: &Path,
    result: &AgentCommandResultV1,
) -> Result<(), RemoteError> {
    ...
    cache
        .entries
        .retain(|entry| entry.command_id != result.command_id);
    cache.entries.push(result.clone());
    if cache.entries.len() > 2048 {
        let drain = cache.entries.len() - 2048;
        cache.entries.drain(0..drain);
    }
...
pub fn remove_cached_agent_result(path: &Path, command_id: &str) -> Result<(), RemoteError> {
    ...
    cache.entries.retain(|entry| entry.command_id != command_id);
...
pub fn refresh_agent_result(
    identity: &DeviceIdentity,
    prior: &AgentCommandResultV1,
) -> Result<AgentCommandResultV1, RemoteError> {
    if prior.device_id != identity.metadata().device_id {
        return Err(RemoteError::DeviceMismatch);
    }
    if !safe_id(&prior.command_id, 16, 128) || !safe_id(&prior.session_id, 16, 256) {
        return Err(RemoteError::InvalidId);
    }
    let mut result = AgentCommandResultV1 {
        version: REMOTE_PROTOCOL_VERSION,
        device_id: prior.device_id.clone(),
        command_id: prior.command_id.clone(),
...
pub fn generate_control_plane_keypair() -> ControlPlaneKeypair {
...
pub fn control_plane_public_key(private_key_b64: &str) -> Result<String, RemoteError> {
    let signing = decode_signing_key(private_key_b64)?;
    Ok(B64.encode(signing.verifying_key().to_bytes()))
}
pub fn sign_remote_command(
    private_key_b64: &str,
    command: &mut RemoteCommandV1,
) -> Result<(), RemoteError> {
    let signing = decode_signing_key(private_key_b64)?;
    command.signature = B64.encode(signing.sign(&command.canonical_bytes()?).to_bytes());
    Ok(())
}
...
impl AgentStreamRelayClient {
    pub fn connect(base_url: &str, identity: &DeviceIdentity) -> Result<Self, RemoteError> {
        let url = stream_gateway_url(base_url, "/v1/agent/streams/ws")?;
        let (socket, _) = tungstenite::connect(url.as_str())
            .map_err(|e| RemoteError::Transport(format!("stream gateway connect failed: {e}")))?;
        let mut out = Self { socket };
        out.send_client(&AgentStreamClientMessageV1::Hello(
            build_agent_stream_hello(identity)?,
        ))?;
        Ok(out)
    }
    pub fn read_server(&mut self) -> Result<AgentStreamServerMessageV1, RemoteError> {
        loop {
            let message = self
                .socket
                .read()
                .map_err(|e| RemoteError::Transport(format!("stream gateway read failed: {e}")))?;
            match message {
                tungstenite::Message::Text(text) => {
                    if text.len() > 2 * 1024 * 1024 {
                        return Err(RemoteError::Transport(
                            "stream gateway message exceeds 2 MiB".into(),
                        ));
                    }
                ...
                tungstenite::Message::Ping(v) => {
                    self.socket
                        .send(tungstenite::Message::Pong(v))
                        .map_err(|e| {
                            RemoteError::Transport(format!("stream gateway pong failed: {e}"))
                        })?;
                }
                tungstenite::Message::Pong(_) | tungstenite::Message::Frame(_) => {}
                tungstenite::Message::Close(_) => {
                    return Err(RemoteError::Transport(
                        "stream gateway closed connection".into(),
                    ))
                }
            }
        }
    }
    pub fn send_client(&mut self, message: &AgentStreamClientMessageV1) -> Result<(), RemoteError> {
        ...
        self.socket
            ...
            .map_err(|e| RemoteError::Transport(format!("stream gateway send failed: {e}")))
    }
}
fn stream_gateway_url(base_url: &str, path: &str) -> Result<String, RemoteError> {
    ...
    if let Some(rest) = base.strip_prefix("http://127.0.0.1") {
        return Ok(format!("ws://127.0.0.1{rest}{path}"));
    }
    if let Some(rest) = base.strip_prefix("http://localhost") {
        return Ok(format!("ws://localhost{rest}{path}"));
    }
    Err(RemoteError::Transport(
        "control plane URL cannot be converted to a secure stream gateway URL".into(),
    ))
}
...
impl WebSocketControlPlaneClient {
    pub fn new(base_url: &str) -> Result<Self, RemoteError> {
        ...
        let (socket, _) = tungstenite::connect(url.as_str())
            .map_err(|e| RemoteError::Transport(format!("gateway connect failed: {e}")))?;
        Ok(Self { socket })
    }
    ...
    pub fn poll(&mut self, poll: &AgentPollV1) -> Result<AgentPollResponseV1, RemoteError> {
        match self.request(&AgentGatewayRequestV1::Poll(poll.clone()))? {
            ...
            other => Err(RemoteError::Transport(format!(
                "unexpected gateway response to poll: {other:?}"
            ))),
        }
    }
    ...
    pub fn submit_result(&mut self, result: &AgentCommandResultV1) -> Result<(), RemoteError> {
        match self.request(&AgentGatewayRequestV1::Result(result.clone()))? {
            ...
            AgentGatewayResponseV1::Ack { ok: false, .. } => Err(RemoteError::Transport(
                "gateway returned negative acknowledgement".into(),
            )),
            AgentGatewayResponseV1::Error { message } => Err(RemoteError::Transport(format!(
                "gateway rejected result: {message}"
            ))),
            other => Err(RemoteError::Transport(format!(
                "unexpected gateway response to result: {other:?}"
            ))),
        }
    }
    ...
    fn request(
        &mut self,
        request: &AgentGatewayRequestV1,
    ) -> Result<AgentGatewayResponseV1, RemoteError> {
        let text = serde_json::to_string(request)?;
        if text.len() > 2 * 1024 * 1024 {
            return Err(RemoteError::Transport(
                "gateway request exceeds 2 MiB safety limit".into(),
            ));
        }
        self.socket
            .send(tungstenite::Message::Text(text.into()))
            .map_err(|e| RemoteError::Transport(format!("gateway send failed: {e}")))?;
        loop {
            let message = self
                .socket
                .read()
                .map_err(|e| RemoteError::Transport(format!("gateway read failed: {e}")))?;
            match message {
                tungstenite::Message::Text(text) => {
                    if text.len() > 2 * 1024 * 1024 {
                        return Err(RemoteError::Transport(
                            "gateway response exceeds 2 MiB safety limit".into(),
                        ));
                    }
                    return serde_json::from_str(text.as_str()).map_err(RemoteError::from);
                }
...
fn gateway_url(base_url: &str) -> Result<String, RemoteError> {
    ...
    let base = base_url.trim_end_matches('/');
    if let Some(rest) = base.strip_prefix("https://") {
        return Ok(format!("wss://{rest}/v1/agent/ws"));
    }
    if let Some(rest) = base.strip_prefix("http://127.0.0.1") {
        return Ok(format!("ws://127.0.0.1{rest}/v1/agent/ws"));
    }
    if let Some(rest) = base.strip_prefix("http://localhost") {
        return Ok(format!("ws://localhost{rest}/v1/agent/ws"));
    }
    Err(RemoteError::Transport(
        "control plane URL cannot be converted to a secure gateway URL".into(),
    ))
}
...
impl HttpControlPlaneClient {
    ...
    pub fn enroll(&self, enrollment: &DeviceEnrollmentV1) -> Result<(), RemoteError> {
        let response = self
            .client
    ...
    pub fn poll(&self, poll: &AgentPollV1) -> Result<AgentPollResponseV1, RemoteError> {
        let response = self
            .client
            .post(format!("{}/v1/agent/poll", self.base_url))
            .json(poll)
            .send()
            .map_err(http_err)?;
    ...
    pub fn submit_result(&self, result: &AgentCommandResultV1) -> Result<(), RemoteError> {
        let response = self
            .client
            .post(format!("{}/v1/agent/result", self.base_url))
            .json(result)
            .send()
            .map_err(http_err)?;
        ensure_success(response)?;
    ...
    pub fn submit_audit(&self, batch: &AgentAuditBatchV1) -> Result<(), RemoteError> {
        let response = self
            .client
            .post(format!("{}/v1/agent/audit", self.base_url))
            .json(batch)
            .send()
            .map_err(http_err)?;
        ensure_success(response)?;
...
fn http_err<E: std::fmt::Display>(e: E) -> RemoteError {
    RemoteError::Transport(e.to_string())
}
fn validate_control_plane_url(value: &str) -> Result<(), RemoteError> {
        ...
        || value.starts_with("http://localhost")
    {
        ...
        Err(RemoteError::Transport(
            "control plane URL must use HTTPS except loopback development".into(),
        ))
    }
}
fn decode_signing_key(value: &str) -> Result<SigningKey, RemoteError> {
    let bytes = B64
        .decode(value)
    ...
    let bytes: [u8; 32] = bytes
        .as_slice()
        .try_into()
        .map_err(|_| RemoteError::Key("private key must be 32 bytes".into()))?;
...
fn enrollment_proof_bytes(
    ...
    Ok(serde_json::to_vec(&Proof {
        version: REMOTE_PROTOCOL_VERSION,
        purpose: "vsn-device-enrollment",
        device_id,
        public_key,
        pairing_nonce,
    })?)
}
fn validate_fresh_timestamp(timestamp: u128) -> Result<(), RemoteError> {
    let now = now_ms();
    if timestamp > now.saturating_add(MAX_CLOCK_SKEW_MS)
        || timestamp.saturating_add(MAX_CLOCK_SKEW_MS) < now
    {
        Err(RemoteError::Timing)
    } else {
        Ok(())
    }
}
fn safe_id(value: &str, min: usize, max: usize) -> bool {
    value.len() >= min
        && value.len() <= max
        && value
            .bytes()
...
mod tests {
    ...
    fn durable_result_cache_can_list_and_ack() {
        ...
        assert!(load_cached_agent_result(&path, &result.command_id)
            .unwrap()
            .is_some());
