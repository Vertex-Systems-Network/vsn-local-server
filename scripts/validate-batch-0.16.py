#!/usr/bin/env python3
"""Static/offline validation for VSN 0.16 source artifacts.
...
try:
    ...
    if roadmap.get('product_version')!='0.16.0': errors.append('roadmap-status.json product_version must be 0.16.0')
    try:
...
# 0.16 integration anchors: static presence only, not compiler proof.
required={
 'crates/vsn-control-store/src/lib.rs':['SharedApiTokenRecord','SharedFleetGroupRecord','vsn_control_api_tokens','vsn_control_environments','upsert_device_fleet'],
 'cloud/control-plane/src/main.rs':['0.16.0','backfill_shared_iam_fleet_once','refresh_shared_iam_fleet_state','sync_shared_api_token','sync_shared_device_fleet'],
 'crates/vsn-update/src/lib.rs':['apply_verified_file_locked','rollback_verified_file_locked','acquire_update_lock','recover_stale_update_lock','UpdateStatus'],
 'apps/updater-helper/src/main.rs':['RecoverLock','apply_verified_file_locked','update_status'],
 'crates/vsn-preview/src/lib.rs':['PreviewEventStreamRequest','start_event_stream','read_event_stream','MAX_PREVIEW_STREAM_BYTES','TrySendError'],
 'cloud/dashboard/src/LivePreview.tsx':["'sse'",'lastEventId','max_duration_seconds','text/event-stream'],
 'crates/vsn-cloud/src/lib.rs':['CloudCliImageCopyRequest','cloud_cli_copy_image','target_location','copy-image'],
 'apps/agent/src/main.rs':['0.16.0','cloud.cli.copy-image','preview_event_stream_start','allow_remote_database_query'],
 'apps/cli/src/main.rs':['0.16.0','cli-copy-image'],
 'contracts/preview-event-stream-v1.schema.json':['VSN Local Preview SSE Relay v1','max_duration_seconds'],
 'contracts/update-helper-v1.schema.json':['VSN Out-of-Process Updater Helper v1','recover_lock'],
 'contracts/roadmap-status-v1.schema.json':['overall_completion_percent','completion_percent'],
 'docs/roadmap-status.json':['0.16.0','overall_completion_percent','completion_percent'],
 'docs/BATCH-0.16.md':['Shared PostgreSQL normalization','vsn-updater-helper','completion_percent'],
 'docs/updater-helper.md':['apply.lock','confirm_recover'],
 'docs/preview-sse.md':['text/event-stream','backpressure'],
 'docs/cloud-image-copy.md':['cli-copy-image','confirm_copy'],
 '.github/workflows/release-gate.yml':['cargo clippy','cargo audit','validate-batch-0.16.py','SOURCE_SHA256SUMS.txt'],
 'scripts/release-gate.py':['0.16.0','SHA256','manifest'],
}
for rel,needles in required.items():
    p=ROOT/rel
    if not p.is_file(): errors.append(f'Missing 0.16 integration file: {rel}'); continue
    text=p.read_text(encoding='utf-8')
    for needle in needles:
        if needle not in text: errors.append(f'Missing 0.16 integration anchor {needle} in {rel}')
