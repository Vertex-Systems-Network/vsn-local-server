#!/usr/bin/env python3
"""Static/offline validation for VSN 0.9 source artifacts.
...
try:
    ...
    if roadmap.get('product_version')!='0.9.0': errors.append('roadmap-status.json product_version must be 0.9.0')
...
# 0.9 integration anchors: static presence only, not compiler proof.
required={
 'crates/vsn-terminal/src/lib.rs':['read_pty_session_wait','read_session_wait'],
 'crates/vsn-database-native/src/lib.rs':['postgres_insert','postgres_indexes','mysql_insert','mysql_relations','redis_set_string','redis_delete'],
 'crates/vsn-files/src/lib.rs':['binary_upload_status','file_digest'],
 'crates/vsn-preview/src/lib.rs':['PreviewHttpRequest','pub fn request'],
 'crates/vsn-cloud/src/lib.rs':['ssh_release_upload','ssh_release_activate','ssh_release_rollback','PREVIOUS'],
 'crates/vsn-auth/src/lib.rs':['create_recovery_codes','create_oidc_pkce_transaction','authorization_endpoint'],
 'crates/vsn-control-store/src/lib.rs':['save_if_generation','GenerationConflict'],
 'cloud/control-plane/src/main.rs':['/v1/auth/oidc/begin','regenerate_account_recovery_codes','state_generation','save_if_generation'],
}
for rel,needles in required.items():
    p=ROOT/rel
    if not p.is_file(): errors.append(f'Missing 0.9 integration file: {rel}'); continue
    text=p.read_text(encoding='utf-8')
    for needle in needles:
        if needle not in text: errors.append(f'Missing 0.9 integration anchor {needle} in {rel}')
