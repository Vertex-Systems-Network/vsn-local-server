#!/usr/bin/env python3
"""Static/offline validation for VSN 0.20 source artifacts.
...
try:
    ...
    if roadmap.get('product_version')!='0.20.0': errors.append('roadmap-status.json product_version must be 0.20.0')
    try:
...
# 0.20 integration anchors: static presence only, not compiler proof.
required={
 ...
 'apps/agent/src/main.rs':['0.20.0','runtime.repair','ai.execute','ai.evaluate','vault.rotate','vault.restore','vault.retire','marketplace.resolve-update-channel','extension.sandbox-capabilities','extension.exec','network.dns-start','resolver-apply','terminal.pty.recovery.list'],
 'apps/cli/src/main.rs':['0.20.0','runtime.repair','ai.evaluate','key-history','resolve-update-channel','sandbox-capabilities','pty-recovery-list'],
 ...
 'cloud/control-plane/src/main.rs':['0.20.0','/ready','/v1/admin/ops','async fn readiness','async fn ops_status','VSN_SLO_CONTROL_P95_MS'],
 'scripts/release-evidence.py':['vault-key-rotation','penetration-test','require-certified','completion_percent'],
 'docs/release-evidence-0.20.json':['vault-key-rotation','penetration-test'],
 ...
 '.github/workflows/release-gate.yml':['validate-batch-0.20.py','release-evidence.py','windows-msi','linux-deb','macos-pkg','cargo audit'],
 ...
 'scripts/smoke-batch-0.20-linux.sh':['cargo clippy','build-deb.sh','smoke-updater-helper.py'],
 'scripts/smoke-batch-0.20-macos.sh':['cargo clippy','build-pkg.sh','smoke-updater-helper.py'],
 'scripts/smoke-batch-0.20-windows.ps1':['cargo clippy','build-msi.ps1','pg-native-job-list'],
 'docs/roadmap-status.json':['0.20.0','overall_completion_percent','completion_percent'],
 'docs/BATCH-0.20.md':['Vault rotation','AI execution','release evidence','.test DNS'],
 'docs/P30-release-readiness-0.20.md':['21-control','release-evidence.py'],
 ...
 'docs/BATCH-0.20.md':['Runtime lifecycle','Vault lifecycle','Bubblewrap','Marketplace channels'],
 'docs/STATUS-0.20.md':['85%'],
 ...
 'crates/vsn-runtime/src/lib.rs':['RuntimeAuditReport','audit_registry'],
 'crates/vsn-database/src/lib.rs':['AdvancedModelRequest','AdvancedModelAnalysis','analyze_advanced_model'],
 'crates/vsn-container/src/lib.rs':['RegistryPushRequest','tag_and_push'],
 'crates/vsn-marketplace/src/lib.rs':['MarketplacePublisher','PublisherState','publishers'],
 'crates/vsn-ai/src/lib.rs':['CandidatePlanValidation','validate_candidate_plan','AiCapabilityReport'],
 'crates/vsn-control-store/src/lib.rs':['SharedTeamSecretRecord','vsn_control_team_secrets','upsert_team_secret'],
 'cloud/control-plane/src/main.rs':['VSN_CONTROL_VAULT_KEY_B64','/v1/admin/vault/secrets','control.vault.reveal'],
 'cloud/dashboard/src/main.tsx':['Control Plane 0.20','TEAM VAULT','Shared encrypted secrets'],
 'scripts/release-preflight.py':['VSN release preflight','ready_for_host_build','Preflight reports runner capability only'],
 'contracts/release-preflight-v1.schema.json':['VSN Release Runner Preflight v1'],
 'docs/BATCH-0.20.md':['Runtime audit','Team Vault','publisher governance','candidate ToolPlan'],
 'docs/STATUS-0.20.md':['87%'],
} 

for rel,needles in required.items():
    p=ROOT/rel
    if not p.is_file(): errors.append(f'Missing 0.20 integration file: {rel}'); continue
    text=p.read_text(encoding='utf-8')
    for needle in needles:
        if needle not in text: errors.append(f'Missing 0.20 integration anchor {needle} in {rel}')
