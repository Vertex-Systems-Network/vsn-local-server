#!/usr/bin/env python3
"""Static/offline validation for VSN 0.8 source artifacts.
...
try:
    ...
    if roadmap.get('product_version')!='0.8.0': errors.append('roadmap-status.json product_version must be 0.8.0')
...
# 0.8 integration anchors: static presence only, not compiler proof.
required={
 'crates/vsn-terminal/src/lib.rs':['start_pty_session','resize_pty_session'],
 'crates/vsn-database-native/src/lib.rs':['postgres_inspect','mysql_inspect','redis_inspect'],
 'crates/vsn-remote/src/lib.rs':['WebSocketControlPlaneClient','AgentGatewayRequestV1'],
 'cloud/control-plane/src/main.rs':['/v1/agent/ws','account_login','update_account','VSN_CONTROL_AUTH_KEY_B64'],
 'crates/vsn-cloud/src/lib.rs':['ssh_workspace_prepare','ssh_workspace_remove_empty'],
}
for rel,needles in required.items():
    p=ROOT/rel
    if not p.is_file(): errors.append(f'Missing 0.8 integration file: {rel}'); continue
    text=p.read_text(encoding='utf-8')
    for needle in needles:
        if needle not in text: errors.append(f'Missing 0.8 integration anchor {needle} in {rel}')
