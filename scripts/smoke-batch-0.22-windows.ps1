...
Set-Location $Root
Write-Host '== VSN 0.22 Windows integration smoke =='
...
try {
  ...
  $Workspace = Join-Path $env:TEMP 'vsn-0.22-smoke'
  ...
  $install = Join-Path $env:TEMP 'vsn-0.22-update-smoke'
  ...
  $staged = Join-Path $env:TEMP 'vsn-0.22-staged.bin'
  ...
  $apply = @{ install_root=$install; target_relative='bin/demo.bin'; staged_artifact=$staged; expected_sha256=$sha; release='0.22.0-smoke'; confirm_apply=$true } | ConvertTo-Json -Compress
  ...
  if (Get-Command python -ErrorAction SilentlyContinue) {
    python scripts/validate-batch-0.22.py
  ...
  if (Get-Command wix -ErrorAction SilentlyContinue) {
    ...
    & (Join-Path $Root 'packaging\windows\build-msi.ps1') -Version 0.22.0 -SourceDir (Join-Path $Root 'target\release') -OutputDir (Join-Path $env:TEMP 'vsn-0.22-msi-smoke')
  }
  ...
  Write-Host 'VSN 0.22 Windows integration smoke completed.'
}
finally {
