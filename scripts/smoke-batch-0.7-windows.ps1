...
Set-Location $Root
Write-Host '== VSN 0.7 Windows integration smoke =='
...
try {
  & $Cli status | Out-Host
  $Workspace = Join-Path $env:TEMP 'vsn-0.7-smoke'
  ...
  # Persistent terminal pipe session. This deliberately validates process pipes,
  # not PTY/ConPTY semantics (those are still a later roadmap item).
  ...
  Write-Host 'VSN 0.7 Windows integration smoke completed.'
}
finally {
