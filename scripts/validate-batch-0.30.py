#!/usr/bin/env python3
"""Offline/source integration gate for VSN 0.30.0. Native compilation remains a separate P30 evidence gate."""
...
try:
 ...
 if d.get('product_version')!='0.30.0': errors.append('roadmap product_version != 0.30.0')
...
for p in sorted(ROOT.rglob('Cargo.toml')):
 try:
  ...
  if 'package' in d:
   ...
   if v not in ('0.30.0','0.0.0'): errors.append(f'package version drift {p.relative_to(ROOT)}={v}')
  tables=[]
...
required={
 ...
 'apps/cli/src/main.rs':['0.30.0','command_catalog','completion_script','diagnostics','commands'],
 ...
 'cloud/control-plane/src/main.rs':['0.30.0','/v1/admin/vault/rotate','loaded_key_ids','active_key_id','delete_fleet_group','delete_environment','validate_fleet'],
 'cloud/control-plane/src/main.rs':['0.30.0','/v1/admin/control/validate','/v1/admin/iam/validate','/v1/admin/security/validate','validate_control_plane','validate_iam','validate_security'],
 ...
 'docs/BATCH-0.30.md':['Candidate-Bound P30 Certification','98.9032%'],
 'docs/source-closure-0.30.md':['P0–P29','P30'],
 'scripts/control-plane-dr.py':['pg_dump','pg_restore','RESTORE_CONTROL_PLANE'],
 'scripts/source-readiness.py':['SOURCE_CLOSED','P30'],
 'packaging/linux/build-deb.sh':['0.30.0'],
 'packaging/macos/build-pkg.sh':['0.30.0'],
 'packaging/windows/build-msi.ps1':['0.30.0'],
 'docs/STATUS-0.30.md':['98.9032%','0/21'],
 '.github/workflows/ci.yml':['validate-batch-0.30.py'],
 '.github/workflows/release-gate.yml':['validate-batch-0.30.py'],
}
...
extra_required={
 ...
 'docs/BATCH-0.30.md':['P30','evidence'],
 'docs/source-closure-0.30.md':['P0–P29','P30'],
 'docs/STATUS-0.30.md':['99%','30/30','P30'],
}
...
for rel,needles in {
 'scripts/release-evidence.py':['schema_version":3','blocked','max_age_days','allow_waivers'],
 ...
 'contracts/release-evidence-v3.schema.json':['candidate_id','blocked','max_age_days'],
 'contracts/p30-progress-v1.schema.json':['p30_completion_percent'],
 'docs/release-evidence-current.json':['"schema_version": 3','"product_version": "0.30.0"','"candidate_id"'],
 '.github/workflows/release-gate.yml':['release-evidence-current.json','scripts/p30-fragment.py'],
 '.github/workflows/release-signing.yml':['signing-evidence','0.30.0'],
}.items():
 p=ROOT/rel
...
extra_027={
 ...
 'docs/BATCH-0.30.md':['candidate_id','different source candidates'],
 'docs/STATUS-0.30.md':['30/30','0/21'],
}
...
for rel,needles in {
 ...
 'scripts/release-evidence.py':['schema_version\":3','candidate_bound','candidate mismatch'],
...
# 0.30 incremental runner evidence / runner-pack anchors
for rel,needles in {
 ...
 p=ROOT/rel
 if not p.is_file(): errors.append(f'missing 0.30 runner evidence file {rel}'); continue
 text=p.read_text(errors='ignore')
 for needle in needles:
  if needle not in text: errors.append(f'missing 0.30 runner evidence anchor {needle!r} in {rel}')
try:
 packs=json.loads((ROOT/'certification/p30-runner-packs.json').read_text())
 if packs.get('product_version')!='0.30.0': errors.append('runner pack product_version != 0.30.0')
 covered=[]
