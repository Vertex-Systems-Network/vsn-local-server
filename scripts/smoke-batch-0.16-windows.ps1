...
Set-Location $Root
Write-Host '== VSN 0.16 Windows integration smoke =='
...
try {
  ...
  if ($version -notmatch '0\.16\.0') { throw "unexpected CLI version: $version" }
  & $Cli status | Out-Host

  $Workspace = Join-Path $env:TEMP 'vsn-0.16-smoke'
  ...
  "echo VSN_PTY_016_OK`r`nexit`r`n" | & $Cli terminal pty-write $pty.session_id | Out-Host
  $ptyOut = ((& $Cli terminal pty-read-wait $pty.session_id 3000) | Out-String) | ConvertFrom-Json
  if ($ptyOut.output -notmatch 'VSN_PTY_016_OK') { throw 'PTY output missing' }
  ...
  if ($scrollText -notmatch 'VSN_PTY_016_OK') { throw 'durable PTY scrollback missing' }

  # Generic stream foundation smoke through authenticated Agent IPC.
  $stream = ('{"kind":"custom","direction":"bidirectional","resource_id":"smoke-016","metadata":{}}' | & $Cli stream open | Out-String) | ConvertFrom-Json
  ...
  $install = Join-Path $env:TEMP 'vsn-0.16-update-smoke'
  ...
  $staged = Join-Path $env:TEMP 'vsn-0.16-staged.bin'
  ...
  $apply = @{ install_root=$install; target_relative='bin/demo.bin'; staged_artifact=$staged; expected_sha256=$sha; release='0.16.0-smoke'; confirm_apply=$true } | ConvertTo-Json -Compress
  ...
  if (Get-Command python -ErrorAction SilentlyContinue) {
    python scripts/validate-batch-0.16.py
  ...
  Write-Host 'VSN 0.16 Windows integration smoke completed.'
}
finally {
