...
Set-Location $Root
Write-Host '== VSN 0.14 Windows integration smoke =='
...
try {
  ...
  $Workspace = Join-Path $env:TEMP 'vsn-0.14-smoke'
  ...
  "echo VSN_PTY_014_OK`r`nexit`r`n" | & $Cli terminal pty-write $pty.session_id | Out-Host
  $ptyOut = ((& $Cli terminal pty-read-wait $pty.session_id 3000) | Out-String) | ConvertFrom-Json
  if ($ptyOut.output -notmatch 'VSN_PTY_014_OK') { throw 'PTY output missing' }
  & $Cli terminal pty-remove $pty.session_id | Out-Host

  # Generic stream foundation smoke through authenticated Agent IPC.
  $stream = ('{"kind":"custom","direction":"bidirectional","resource_id":"smoke-014","metadata":{}}' | & $Cli stream open | Out-String) | ConvertFrom-Json
  ...
  if (Get-Command python -ErrorAction SilentlyContinue) {
    python scripts/validate-batch-0.14.py
  ...
  Write-Host 'VSN 0.14 Windows integration smoke completed.'
}
finally {
