#!/usr/bin/env python3
"""Offline/source integration gate for VSN 0.22.0. Native compilation remains a separate P30 evidence gate."""
...
try:
 ...
 if d.get('product_version')!='0.22.0': errors.append('roadmap product_version != 0.22.0')
 ...
 for closed in ('P0','P1','P2','P3','P6','P7','P11','P12','P19','P20','P21','P22','P24','P29'):
...
for p in sorted(ROOT.rglob('Cargo.toml')):
 try:
  ...
  if 'package' in d:
   ...
   if v not in ('0.22.0','0.0.0'): errors.append(f'package version drift {p.relative_to(ROOT)}={v}')
  tables=[]
...
required={
 ...
 'apps/cli/src/main.rs':['0.22.0','command_catalog','completion_script','diagnostics','commands'],
 ...
 'cloud/control-plane/src/main.rs':['0.22.0','/v1/admin/vault/rotate','loaded_key_ids','active_key_id','delete_fleet_group','delete_environment','validate_fleet'],
 'cloud/control-plane/src/main.rs':['0.22.0','/v1/admin/control/validate','/v1/admin/iam/validate','/v1/admin/security/validate','validate_control_plane','validate_iam','validate_security'],
 ...
 'docs/BATCH-0.22.md':['maximum close-first','P29'],
 'docs/source-closure-0.22.md':['P3','P6','P12','P19','P21','P29'],
 'scripts/control-plane-dr.py':['pg_dump','pg_restore','RESTORE_CONTROL_PLANE'],
 'scripts/source-readiness.py':['SOURCE_CLOSED','P30'],
 'packaging/linux/build-deb.sh':['0.22.0'],
 'packaging/macos/build-pkg.sh':['0.22.0'],
 'packaging/windows/build-msi.ps1':['0.22.0'],
 'docs/STATUS-0.22.md':['91%'],
 '.github/workflows/ci.yml':['validate-batch-0.22.py'],
 '.github/workflows/release-gate.yml':['validate-batch-0.22.py'],
}
