...
Set-Location $Root
Write-Host '== VSN 0.12 Windows integration smoke =='
...
try {
  ...
  if ($version -notmatch '0\.12\.0') { throw "unexpected CLI version: $version" }
  & $Cli status | Out-Host

  $Workspace = Join-Path $env:TEMP 'vsn-0.12-smoke'
  ...
  "echo VSN_PTY_012_OK`r`nexit`r`n" | & $Cli terminal pty-write $pty.session_id | Out-Host
  $ptyOut = ((& $Cli terminal pty-read-wait $pty.session_id 3000) | Out-String) | ConvertFrom-Json
  if ($ptyOut.output -notmatch 'VSN_PTY_012_OK') { throw 'PTY output missing' }
  & $Cli terminal pty-remove $pty.session_id | Out-Host

  # Generic stream foundation smoke through authenticated Agent IPC.
  $stream = ('{"kind":"custom","direction":"bidirectional","resource_id":"smoke-012","metadata":{}}' | & $Cli stream open | Out-String) | ConvertFrom-Json
  ...
  Write-Host 'VSN 0.12 Windows integration smoke completed.'
}
finally {
