...
Set-Location $Root
Write-Host '== VSN 0.20 Windows integration smoke =='
...
try {
  ...
  $Workspace = Join-Path $env:TEMP 'vsn-0.20-smoke'
  ...
  $install = Join-Path $env:TEMP 'vsn-0.20-update-smoke'
  ...
  $staged = Join-Path $env:TEMP 'vsn-0.20-staged.bin'
  ...
  $apply = @{ install_root=$install; target_relative='bin/demo.bin'; staged_artifact=$staged; expected_sha256=$sha; release='0.20.0-smoke'; confirm_apply=$true } | ConvertTo-Json -Compress
  ...
  if (Get-Command python -ErrorAction SilentlyContinue) {
    python scripts/validate-batch-0.20.py
    python scripts/validate-schemas.py
  }

  if (Get-Command wix -ErrorAction SilentlyContinue) {
    cargo build -p vsn-agent -p vsn -p vsn-updater-helper --release
    & (Join-Path $Root 'packaging\windows\build-msi.ps1') -Version 0.20.0 -SourceDir (Join-Path $Root 'target\release') -OutputDir (Join-Path $env:TEMP 'vsn-0.20-msi-smoke')
  }
  ...
  Write-Host 'VSN 0.20 Windows integration smoke completed.'
}
finally {
