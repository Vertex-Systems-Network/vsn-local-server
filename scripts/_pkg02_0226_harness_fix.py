...
p = Path('scripts/self-hosted/pkg02-0226.ps1')
...
once('''function Write-FakeClient([string]$Name, [string]$Body) {
...
}''', 'fake client newline expansion')

once('''$outsideCa = Join-Path $outside 'outside-ca.pem'
$junctionCa = Join-Path $junction 'outside-ca.pem'
$script:FakeLog''', '''$outsideCa = Join-Path $outside 'outside-ca.pem'
$junctionCa = Join-Path $junction 'outside-ca.pem'
$outsideCredential = Join-Path $outside 'outside-client.cnf'
$junctionCredential = Join-Path $junction 'outside-client.cnf'
$script:FakeLog''', 'credential fixture variables')

once('''$fakeDetectionElapsedMs = 0L
$payloadZipSha = $null''', '''$fakeDetectionElapsedMs = 0L
$slowDetectionElapsedMs = 0L
$noisyDetectionElapsedMs = 0L
$payloadZipSha = $null''', 'detection measurements')

once('''Set-Content -LiteralPath $outsideCa -Value 'outside-root-ca' -Encoding ascii
    New-Item -ItemType Junction''', '''Set-Content -LiteralPath $outsideCa -Value 'outside-root-ca' -Encoding ascii
    Set-Content -LiteralPath $outsideCredential -Value "[client]`npassword=outside-secret" -Encoding ascii
    New-Item -ItemType Junction''', 'outside credential fixture')

once('''    $fiveEngineCount = $clientRows.Count

    $pgLocal = Invoke-CliJson''', '''    $fiveEngineCount = $clientRows.Count
    ...
    $pgLocal = Invoke-CliJson''', 'hostile detection fixtures')

once('''    $mariaLocal = Invoke-CliJson @('db','inspect','mariadb','::1','3306','fixture','app') 'maria-local-inspect'
    foreach ($item in @($pgLocal,$mysqlLocal,$mariaLocal)) {''', '''    $mariaLocal = Invoke-CliJson @('db','inspect','mariadb','::1','3306','fixture','app') 'maria-local-inspect'
    $loopbackExtra = @(
    ...
    foreach ($item in @($pgLocal,$mysqlLocal,$mariaLocal) + $loopbackExtra) {''', 'exact loopback positive matrix')

once('''    Assert-NoFakeSpawn {
    ...
    Assert-NoFakeSpawn {''', '''    Assert-NoFakeSpawn {
        $f = Invoke-CliFailure @('db','inspect','postgresql','db1.example.test,db2.example.test','5432','fixture','app') 'reject-multihost'
        Assert-FailureContains $f 'unambiguous' 'reject-multihost'
    } 'reject-multihost'
    Assert-NoFakeSpawn {
    ...
    Assert-NoFakeSpawn {''', 'userinfo spoof rejection')
...
once('''    Assert-NoFakeSpawn {
    ...
    $pgCredential = Invoke-CliJson''', '''    Assert-NoFakeSpawn {
        $missing = Join-Path $workspace 'missing-ca.pem'
        $f = Invoke-CliFailure @('db','inspect-tls','postgresql','db.example.test','5432','fixture','app',$missing) 'reject-missing-ca'
        Assert-FailureContains $f 'unavailable' 'reject-missing-ca'
    } 'reject-missing-ca'
    ...
    $pgCredential = Invoke-CliJson''', 'credential containment matrix')

once('''    $pgCredential = Invoke-CliJson @('db','inspect-tls','postgresql','db.example.test','5432','fixture','app',$ca,$credential) 'pg-tls-credential'
    if ([string]$pgCredential.engine -ne 'postgresql') { throw 'contained credential file did not succeed' }''', '''    Assert-NoFakeSpawn {
    ...
    if ([string]$pgCredential.engine -ne 'postgresql') { throw 'contained credential file did not succeed' }''', 'unknown engine rejection')

once('''        client_detection_total_ms = $fakeDetectionElapsedMs
        external_stdout_limit_bytes''', '''        client_detection_total_ms = $fakeDetectionElapsedMs
        slow_client_detection_total_ms = $slowDetectionElapsedMs
        noisy_client_detection_total_ms = $noisyDetectionElapsedMs
        external_stdout_limit_bytes''', 'detection evidence measurements')
