#!/usr/bin/env python3
"""Offline/source integration gate for VSN 0.37.0 with P30 handoff/resume certification. Native compilation remains a separate P30 evidence gate."""
...
try:
 ...
 if d.get('product_version')!='0.37.0': errors.append('roadmap product_version != 0.37.0')
...
for p in sorted(ROOT.rglob('Cargo.toml')):
 try:
  ...
  if 'package' in d:
   ...
   if v not in ('0.37.0','0.0.0'): errors.append(f'package version drift {p.relative_to(ROOT)}={v}')
  tables=[]
...
required={
 ...
 'apps/cli/src/main.rs':['0.37.0','command_catalog','completion_script','diagnostics','commands'],
 ...
 'cloud/control-plane/src/main.rs':['0.37.0','/v1/admin/vault/rotate','loaded_key_ids','active_key_id','delete_fleet_group','delete_environment','validate_fleet'],
 'cloud/control-plane/src/main.rs':['0.37.0','/v1/admin/control/validate','/v1/admin/iam/validate','/v1/admin/security/validate','validate_control_plane','validate_iam','validate_security'],
 ...
 'docs/BATCH-0.37.md':['P30 Runner-Pack Execution','98.9032%'],
 'docs/source-closure-0.37.md':['P0–P29','P30'],
 'scripts/control-plane-dr.py':['pg_dump','pg_restore','RESTORE_CONTROL_PLANE'],
 'scripts/source-readiness.py':['SOURCE_CLOSED','P30'],
 'packaging/linux/build-deb.sh':['0.37.0'],
 'docs/STATUS-0.37.md':['98.9032%','0/21','runner packs'],
 '.github/workflows/ci.yml':['validate-batch-0.37.py'],
 '.github/workflows/release-gate.yml':['validate-batch-0.37.py'],
}
...
extra_required={
 ...
 'docs/BATCH-0.37.md':['P30','evidence'],
 'docs/source-closure-0.37.md':['P0–P29','P30'],
 'docs/STATUS-0.37.md':['99%','30/30','P30'],
}
...
for rel,needles in {
 ...
 'docs/release-evidence-current.json':['"schema_version": 4','"product_version": "0.37.0"','"candidate_id"'],
 '.github/workflows/release-gate.yml':['release-evidence-current.json','scripts/p30-fragment.py'],
 '.github/workflows/release-signing.yml':['signing-evidence','0.37.0'],
}.items():
 p=ROOT/rel
...
extra_027={
 ...
 'docs/BATCH-0.37.md':['candidate_id','different source candidates'],
 'docs/STATUS-0.37.md':['30/30','0/21'],
}
...
try:
 ...
 if packs.get('product_version')!='0.37.0': errors.append('runner pack product_version != 0.37.0')
 covered=[]
...
# 0.37 runner attestation / portable certification anchors
for rel,needles in {
 ...
 p=ROOT/rel
 if not p.is_file(): errors.append(f'missing 0.37 certification file {rel}'); continue
 text=p.read_text(errors='ignore')
 for needle in needles:
  if needle not in text: errors.append(f'missing 0.37 certification anchor {needle!r} in {rel}')
# 0.37 governance anchors
for rel,needles in {
 ...
 'docs/BATCH-0.37.md':['quarantine','supersede','checkpoint'],
 'docs/STATUS-0.37.md':['0/21','98.9032%'],
}.items():
 p=ROOT/rel
 if not p.is_file(): errors.append(f'missing 0.37 governance file {rel}'); continue
 text=p.read_text(errors='ignore')
 for needle in needles:
  if needle not in text: errors.append(f'missing 0.37 governance anchor {needle!r} in {rel}')
# no generated dirs
...
# 0.37 P30 handoff/dispatch/resume anchors
for rel,needles in {
 ...
 'docs/BATCH-0.37.md':['P30 handoff/resume sprint'],
 'docs/STATUS-0.37.md':['98.9032%','0/21'],
}.items():
 p=ROOT/rel
 if not p.is_file(): errors.append(f'missing 0.37 handoff file {rel}'); continue
 text=p.read_text(errors='ignore')
 for needle in needles:
  if needle not in text: errors.append(f'missing 0.37 handoff anchor {needle!r} in {rel}')
