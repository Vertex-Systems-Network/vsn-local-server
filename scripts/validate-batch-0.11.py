#!/usr/bin/env python3
"""Static/offline validation for VSN 0.11 source artifacts.
...
try:
    ...
    if roadmap.get('product_version')!='0.11.0': errors.append('roadmap-status.json product_version must be 0.11.0')
    try:
...
# 0.11 integration anchors: static presence only, not compiler proof.
required={
 'crates/vsn-remote/src/lib.rs':['AgentStreamHelloV1','BrowserStreamClientMessageV1','AgentStreamRelayClient','resource_id:Option<String>'],
 'crates/vsn-control-store/src/lib.rs':['PostgresSnapshotStore','heartbeat_instance','upsert_route','route_owner','vsn_control_instances'],
 'crates/vsn-auth/src/lib.rs':['client_secret_env','mfa_assured','OidcProviderConfig'],
 'cloud/control-plane/src/main.rs':['/v1/agent/streams/ws','/v1/streams/ws','oidc_callback','CoreProviderMetadata::discover_async','id_token_verifier','mapping_required','/v1/admin/cluster','route_owner'],
 'cloud/dashboard/src/LiveTerminal.tsx':['/v1/streams/ws',"resource_id:'new'",'Open live terminal','payload_base64'],
 'cloud/dashboard/src/main.tsx':['linkOidc','/v1/admin/auth/oidc/link','/v1/admin/cluster','LiveTerminal'],
 'apps/agent/src/main.rs':['stream_relay_loop','terminal_pty_start','FileUpload','FileDownload','preview_fetch'],
 'apps/cli/src/main.rs':['0.11.0','cmd=="stream"'],
 'contracts/remote-stream-relay-v1.schema.json':['VSN Remote Stream Relay v1','file_upload','bidirectional'],
 'contracts/oidc-provider-v1.schema.json':['client_secret_env','mfa_assured'],
}
for rel,needles in required.items():
    p=ROOT/rel
    if not p.is_file(): errors.append(f'Missing 0.11 integration file: {rel}'); continue
    text=p.read_text(encoding='utf-8')
    for needle in needles:
        if needle not in text: errors.append(f'Missing 0.11 integration anchor {needle} in {rel}')
