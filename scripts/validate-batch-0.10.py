#!/usr/bin/env python3
"""Static/offline validation for VSN 0.10 source artifacts.
...
try:
    ...
    if roadmap.get('product_version')!='0.10.0': errors.append('roadmap-status.json product_version must be 0.10.0')
    try:
...
# 0.10 integration anchors: static presence only, not compiler proof.
required={
 'crates/vsn-stream/src/lib.rs':['MAX_STREAMS','StreamKind','open_stream','accept_input_frame','queue_output','pull_output'],
 'crates/vsn-database-native/src/lib.rs':['MongoConnection','mongo_inspect','mongo_insert','mongo_indexes','PostgresTlsConnection','postgres_tls_read_query','MySqlTlsConnection','mysql_tls_read_query'],
 'crates/vsn-cloud/src/lib.rs':['SshReleaseHealthRequest','ssh_release_healthcheck','ssh_release_rollback'],
 'crates/vsn-control-store/src/lib.rs':['PostgresSnapshotStore','pg_advisory_xact_lock','save_if_generation','GenerationConflict'],
 'cloud/control-plane/src/main.rs':['/v1/auth/passkey/register/begin','/v1/auth/passkey/login/begin','start_passkey_registration','finish_passkey_authentication','VSN_CONTROL_POSTGRES_DSN','VSN_WEBAUTHN_RP_ID'],
 'cloud/dashboard/src/main.tsx':['registerPasskey','loginPasskey','navigator.credentials.create','navigator.credentials.get'],
 'apps/agent/src/main.rs':['database.native.mongo.inspect','database.tls.postgres.inspect','stream.open','stream.pull'],
 'apps/cli/src/main.rs':['cmd=="stream"','0.10.0','ssh-release-health'],
 'contracts/stream-v1.schema.json':['VSN Bounded Stream Protocol v1','file_upload','bidirectional'],
}
for rel,needles in required.items():
    p=ROOT/rel
    if not p.is_file(): errors.append(f'Missing 0.10 integration file: {rel}'); continue
    text=p.read_text(encoding='utf-8')
    for needle in needles:
        if needle not in text: errors.append(f'Missing 0.10 integration anchor {needle} in {rel}')
