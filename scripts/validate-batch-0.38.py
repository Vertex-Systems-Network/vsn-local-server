#!/usr/bin/env python3
"""Offline/source integration gate for VSN 0.38.0 with P30 handoff/resume certification. Native compilation remains a separate P30 evidence gate."""
...
try:
 ...
 if d.get('product_version')!='0.38.1': errors.append('roadmap product_version != 0.38.1')
...
for p in sorted(ROOT.rglob('Cargo.toml')):
 try:
  ...
  if 'package' in d:
   ...
   if v not in ('0.38.1','0.0.0'): errors.append(f'package version drift {p.relative_to(ROOT)}={v}')
  tables=[]
...
required={
 ...
 'apps/cli/src/main.rs':['0.38.1','command_catalog','completion_script','diagnostics','commands'],
 ...
 'cloud/control-plane/src/main.rs':['0.38.1','/v1/admin/vault/rotate','loaded_key_ids','active_key_id','delete_fleet_group','delete_environment','validate_fleet'],
 'cloud/control-plane/src/main.rs':['0.38.1','/v1/admin/control/validate','/v1/admin/iam/validate','/v1/admin/security/validate','validate_control_plane','validate_iam','validate_security'],
 ...
 'docs/BATCH-0.38.md':['P30 Runner-Pack Execution','98.9032%'],
 'docs/source-closure-0.38.md':['P0–P29','P30'],
 'scripts/control-plane-dr.py':['pg_dump','pg_restore','RESTORE_CONTROL_PLANE'],
 'scripts/source-readiness.py':['SOURCE_CLOSED','P30'],
 'packaging/linux/build-deb.sh':['0.38.1'],
 'docs/STATUS-0.38.md':['98.9032%','0/21','runner packs'],
 '.github/workflows/ci.yml':['validate-batch-0.38.py'],
 '.github/workflows/release-gate.yml':['validate-batch-0.38.py'],
}
...
extra_required={
 ...
 'docs/BATCH-0.38.md':['P30','evidence'],
 'docs/source-closure-0.38.md':['P0–P29','P30'],
 'docs/STATUS-0.38.md':['99%','30/30','P30'],
}
...
for rel,needles in {
 ...
 'docs/release-evidence-current.json':['"schema_version": 4','"product_version": "0.38.1"','"candidate_id"'],
 '.github/workflows/release-gate.yml':['release-evidence-current.json','scripts/p30-fragment.py'],
 '.github/workflows/release-signing.yml':['signing-evidence','0.38.1'],
}.items():
 p=ROOT/rel
...
extra_027={
 ...
 'docs/BATCH-0.38.md':['candidate_id','different source candidates'],
 'docs/STATUS-0.38.md':['30/30','0/21'],
}
...
for rel,needles in {
 ...
 '.github/workflows/release-gate.yml':['scripts/p30-fragment.py','toolchain: 1.97.1'],
...
try:
 ...
 if packs.get('product_version')!='0.38.1': errors.append('runner pack product_version != 0.38.1')
 covered=[]
...
# 0.38 runner attestation / portable certification anchors
for rel,needles in {
 ...
 'certification/linux-core.Dockerfile':['rust:1.97.1-bookworm','node:22-bookworm','cargo-audit','cargo-fuzz'],
 ...
 p=ROOT/rel
 if not p.is_file(): errors.append(f'missing 0.38 certification file {rel}'); continue
 text=p.read_text(errors='ignore')
 for needle in needles:
  if needle not in text: errors.append(f'missing 0.38 certification anchor {needle!r} in {rel}')
# 0.38 governance anchors
for rel,needles in {
 'scripts/p30-evidence-governance.py':['quarantine','supersede','restore-checkpoint','checkpoint','p30-evidence-store'],
 'scripts/p30-result-bundle.py':['build','verify'],
 'scripts/p30_result_lib.py':['safe_extract','P30_RESULT.json','bundle_sha256'],
 'scripts/p30-sync-status.py':['release-evidence-current-report.json','p30-certification-status.json'],
 'scripts/p30-evidence-policy.py':['expiring_soon','require-no-expired'],
 'contracts/p30-evidence-journal-v3.schema.json':['Evidence Journal v3','schema_version'],
 'contracts/p30-evidence-policy-v1.schema.json':['Evidence Aging Policy','policy_ok'],
 'docs/BATCH-0.38.md':['quarantine','supersede','checkpoint'],
 'docs/STATUS-0.38.md':['0/21','98.9032%'],
}.items():
 p=ROOT/rel
 if not p.is_file(): errors.append(f'missing 0.38 governance file {rel}'); continue
 text=p.read_text(errors='ignore')
 for needle in needles:
  if needle not in text: errors.append(f'missing 0.38 governance anchor {needle!r} in {rel}')
# PKG-01 Rust 1.97.1 security point-release pin
for rel,needles in {
 'rust-toolchain.toml':['1.97.1','rustfmt','clippy'],
 'certification/pkg01-linux-core-v1.json':['1.97.1','2026-07-16','88f28fa9af20594179f85d6df67078dfd6fa93e2f6da5e1e9b0ac4997988ca4f'],
 'scripts/pkg01-bootstrap-linux.sh':['OFFICIAL_RUST_SHA256','1.97.1','pinned official'],
 '.github/workflows/pkg01-linux-core.yml':['1.97.1'],
}.items():
 p=ROOT/rel
 if not p.is_file(): errors.append(f'missing PKG-01 pin file {rel}'); continue
 text=p.read_text(errors='ignore')
 for needle in needles:
  if needle not in text: errors.append(f'missing PKG-01 pin anchor {needle!r} in {rel}')
# no generated dirs
...
# 0.38 P30 handoff/dispatch/resume anchors
for rel,needles in {
 'scripts/p30-handoff.py':['SOURCE_SHA256SUMS.txt','source manifest mismatch','P30_HANDOFF.json','bundle_sha256'],
 'scripts/p30-verify-handoff.py':['ZIP CRC failure','source manifest digest mismatch','candidate fingerprint mismatch','--execute'],
 '.github/workflows/p30-handoff.yml':['p30-handoff.py','p30-verify-handoff.py','actions/upload-artifact@v4'],
 'scripts/p30-gh-dispatch.py':['workflow','run','run','watch','run','download','release-evidence.py'],
 'scripts/p30-run-pack.py':['--resume-ledger','PROTECTED_PASS_IDS','resume ledger candidate mismatch'],
 'contracts/p30-handoff-v1.schema.json':['P30 Certification Handoff','source_manifest_sha256','launch'],
 'scripts/test-p30-handoff.py':['p30 handoff regression: PASS'],
 'docs/p30-handoff.md':['p30-handoff.py','p30-gh-dispatch.py','--resume-ledger'],
 'docs/BATCH-0.38.md':['P30 handoff/resume sprint'],
 'docs/STATUS-0.38.md':['98.9032%','0/21'],
}.items():
 p=ROOT/rel
 if not p.is_file(): errors.append(f'missing 0.38 handoff file {rel}'); continue
 text=p.read_text(errors='ignore')
 for needle in needles:
  if needle not in text: errors.append(f'missing 0.38 handoff anchor {needle!r} in {rel}')

print(json.dumps({'ok':not errors,'counts':counts,'errors':errors},indent=2));sys.exit(1 if errors else 0)

# PKG-01 Linux Core execution package invariants
for rel,needles in {
 'certification/pkg01-linux-core-v1.json':['PKG-01','linux-core','all-controls-valid-pass','Cargo.lock'],
 'contracts/pkg01-linux-core-status-v1.schema.json':['PKG-01 Linux Core Status','required_passes'],
 'scripts/pkg01-linux-core.py':['PKG-01','prepare','execute','p30-evidence-governance.py','release-gate.py'],
 'scripts/pkg01-bootstrap-linux.sh':['VSN_PKG01_ALLOW_NETWORK','Cargo.lock','package-lock.json','cargo-audit'],
 'scripts/pkg01-finalize.py':['PKG01-COMPLETE','valid_passes','release-gate.py'],
 'scripts/test-pkg01-linux-core.py':['PKG-01 Linux Core regression: PASS','required_passes'],
 '.github/workflows/pkg01-linux-core.yml':['pkg01-linux-core.py prepare --allow-network','pkg01-finalize.py'],
}.items():
 p=ROOT/rel
 if not p.is_file(): errors.append(f'missing PKG-01 file {rel}'); continue
 text=p.read_text(errors='ignore')
 for needle in needles:
  if needle not in text: errors.append(f'missing PKG-01 anchor {needle!r} in {rel}')
