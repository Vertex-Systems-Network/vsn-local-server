#!/usr/bin/env python3
"""Static/offline validation for VSN 0.12 source artifacts.
...
try:
    ...
    if roadmap.get('product_version')!='0.12.0': errors.append('roadmap-status.json product_version must be 0.12.0')
    try:
...
# 0.12 integration anchors: static presence only, not compiler proof.
required={
 'crates/vsn-remote/src/lib.rs':['STREAM_RELAY_PROTOCOL_VERSION:u32=2','BrowserStreamResumeV1','InputAck','STREAM_RELAY_RESUME_TTL_MS'],
 'crates/vsn-control-store/src/lib.rs':['vsn_control_bus','vsn_control_commands','FOR UPDATE SKIP LOCKED','vsn_control_approvals','approve_and_enqueue','vsn_control_audit','append_audit_batch','vsn_control_rate_limits','consume_rate_limit'],
 'crates/vsn-cloud/src/lib.rs':['CloudCliProvider','cloud_cli_create','cloud_cli_start','cloud_cli_stop','cloud_cli_destroy','--no-associate-public-ip-address','--public-ip-address','--no-address'],
 'cloud/control-plane/src/main.rs':['run_cluster_stream_bus','resume_token','acked_input_seq','approve_and_enqueue','append_audit_batch','consume_rate_limit','all_device_records','STREAM_RELAY_RESUME_TTL_MS'],
 'cloud/dashboard/src/LiveTerminal.tsx':['input_ack','resume_token','pending'],
 'cloud/dashboard/src/LiveFileTransfer.tsx':['MAX_BROWSER_FILE','input_ack','next_input_seq','resume'],
 'cloud/dashboard/src/LiveDatabase.tsx':["kind:'database'",'Workspace SQLite read stream'],
 'cloud/dashboard/src/LivePreview.tsx':["kind:'preview'",'sandbox=""','GET','HEAD'],
 'apps/agent/src/main.rs':['stream_expected_input_seq','InputAck','StreamKind::Database','send_relay_output_chunked'],
 'apps/cli/src/main.rs':['0.12.0','cli-create','cli-start','cli-stop','cli-destroy'],
 'contracts/remote-stream-relay-v2.schema.json':['VSN Remote Stream Relay v2','database','file_upload','preview'],
 'providers/examples/cloud-aws-cli/manifest.json':['aws-cli','provision','destroy'],
 'providers/examples/cloud-azure-cli/manifest.json':['azure-cli','provision','destroy'],
 'providers/examples/cloud-gcp-cli/manifest.json':['gcp-cli','provision','destroy'],
}
for rel,needles in required.items():
    p=ROOT/rel
    if not p.is_file(): errors.append(f'Missing 0.12 integration file: {rel}'); continue
    text=p.read_text(encoding='utf-8')
    for needle in needles:
        if needle not in text: errors.append(f'Missing 0.12 integration anchor {needle} in {rel}')
