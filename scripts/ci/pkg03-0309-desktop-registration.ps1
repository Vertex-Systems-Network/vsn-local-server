...
function Test-ShortcutTarget([object]$Info,[string]$ExpectedExe) {
  ...
  try {
    [IO.Path]::GetFullPath($Info.target).TrimEnd('\') -ieq
      [IO.Path]::GetFullPath($ExpectedExe).TrimEnd('\')
  } catch {
    $false
  }
}
...
function Get-RelevantWindows([int]$RootPid) {
  ...
  foreach ($element in $all) {
    try {
      ...
      if ($visible -and $handle -ne 0 -and ($ids.Contains($pid) -or $titleFallback)) {
        $result += $element
      }
    } catch {}
  }
...
function Record-Window([string]$Phase,[System.Windows.Automation.AutomationElement]$Window) {
  ...
  $pid = 0
  try { $pid = [int]$Window.Current.ProcessId } catch {}
  $summary = @($controls | Where-Object { $_.name } | ForEach-Object { "$($_.control_type):$($_.name)" })
  $fingerprint = "$Phase|$pid|$title|$($summary -join '|')"
...
function Invoke-DesktopShortcutAction(
  [System.Windows.Automation.AutomationElement]$Window,
  [string]$Phase
) {
  $all = $Window.FindAll(
  ...
  foreach ($element in $all) {
    try {
      if (-not [bool]$element.Current.IsEnabled -or [bool]$element.Current.IsOffscreen) { continue }
      $name = Get-SafeName $element
      ...
      try {
        ...
        [void]$Actions.Add([pscustomobject][ordered]@{
          phase = $Phase
          action = 'select-desktop-shortcut-toggle'
          control = $name
          at_utc = [DateTime]::UtcNow.ToString('o')
        })
        Write-UiArtifacts
      ...
      try {
        ...
        [void]$Actions.Add([pscustomobject][ordered]@{
          phase = $Phase
          action = 'select-desktop-shortcut-invoke'
          control = $name
          at_utc = [DateTime]::UtcNow.ToString('o')
        })
        Write-UiArtifacts
...
function Invoke-TerminalFallback(
  ...
  $GA_ROOT = [uint32]2
  $rootHandle = [Vsn0309NativeUi]::GetAncestor($buttonHandle,$GA_ROOT)
  ...
  if ($controlId -gt 0) {
    ...
    [void][Vsn0309NativeUi]::SendMessage($rootHandle,$WM_COMMAND,[IntPtr]$controlId,$buttonHandle)
    [void]$Actions.Add([pscustomobject][ordered]@{
      phase = $Phase
  ...
  if ([Vsn0309NativeUi]::IsWindow($rootHandle)) {
    ...
    [void][Vsn0309NativeUi]::PostMessage($rootHandle,$WM_CLOSE,[IntPtr]::Zero,[IntPtr]::Zero)
    [void]$Actions.Add([pscustomobject][ordered]@{
      phase = $Phase
  ...
  while ([DateTime]::UtcNow -lt $deadline) {
      ...
      if ($completionNow) {
        ...
        if ($quiet -ge 3) {
          ...
          return [pscustomobject]@{
            visible = $visible
            desktop_option_selected = $desktopSelected
    ...
    foreach ($window in $windows) {
      ...
      if ($SelectDesktop -and -not $desktopSelected) {
        if (Invoke-DesktopShortcutAction $window $Phase) {
      ...
      [void](Invoke-Primary $window $Phase $terminalReady)
...
function Get-MsiProperty([string]$Path,[string]$Property) {
  ...
  $view = $db.GetType().InvokeMember(
    'OpenView','InvokeMethod',$null,$db,@("SELECT `Value` FROM `Property` WHERE `Property`='$Property'")
  )
...
function Assert-ShortcutSet(
  [string]$Phase,
  [string]$ExpectedRoot,
  [bool]$RequireDesktop,
  [bool]$RequireStartAumid
) {
  ...
  if ($RequireDesktop) {
    Assert-Condition ($desk.Count -ge 1) "$Phase Desktop shortcut missing."
  }
  ...
  Assert-Condition (
    ($startInfo | Where-Object { Test-ShortcutTarget $_ $exe }).Count -ge 1
  ) "$Phase Start Menu target mismatch."

  if ($RequireDesktop) {
    Assert-Condition (
      ($deskInfo | Where-Object { Test-ShortcutTarget $_ $exe }).Count -ge 1
    ) "$Phase Desktop target mismatch."
  }
...
Assert-Condition $nsisInstall.desktop_option_selected 'NSIS Desktop shortcut GUI option/action was not observed and selected.'
...
if ($tracked.Count -ne 0) {
  $tracked | Write-Host
  throw 'Tracked repository drift detected during 03.09 desktop registration lifecycle.'
}
