...
def require_tokens(text: str, tokens: tuple[str, ...], label: str) -> None:
    for token in tokens:
        if token not in text:
            fail(f"{label} missing frozen token: {token}")


def main() -> None:
    required_files = (
        WINDOWS_CONFIG, NSIS_HOOK, WIX_FRAGMENT, HARNESS, WORKFLOW,
        MANIFEST, TRACKER, STATUS, CHECKPOINT, OWNERSHIP,
    )
    ...
    if run("git", "merge-base", "--is-ancestor", PAUSE_LIFT_HEAD, "HEAD", check=False).returncode:
        fail("exact pause-lift gate head is not an ancestor of implementation head")

    changed = {
        line for line in run("git", "diff", "--name-only", f"{PAUSE_LIFT_HEAD}..HEAD").stdout.splitlines()
        if line
    }
    ...
    if (
        ...
        "MISSING_IMPLEMENTATION", "TASK", "COORDINATED_PARALLEL",
    ):
        fail("manifest identity/classification/authority drifted")

    budget = manifest.get("preflight", {}).get("scope_budget", {})
    max_changed = int(budget.get("max_changed_files", 0))
    max_new = int(budget.get("max_new_files", 0))
    if len(changed) > max_changed:
        fail(f"changed-file budget exceeded: {len(changed)} > {max_changed}")
    if len(added) > max_new:
        fail(f"new-file budget exceeded: {len(added)} > {max_new}")
    ...
    if checkpoint.get("pause", {}).get("product_development_paused") is not False:
        fail("implementation is not descended from explicit unpaused checkpoint")
    ...
    xml_tree = ET.parse(WIX_FRAGMENT)
    xml_root = xml_tree.getroot()
    local = lambda tag: tag.rsplit("}", 1)[-1]
    if any(local(el.tag) in {"File", "Component", "RegistryKey", "RegistryValue"} for el in xml_root.iter()):
        fail("WiX fragment owns files/components/registry state")
    feature = next(
        (el for el in xml_root.iter() if local(el.tag) == "Feature" and el.attrib.get("Id") == "Pkg0311AgentServiceLifecycle"),
        None,
    )
    if feature is None:
        fail("WiX linker anchor feature missing")

    actions = {el.attrib.get("Id"): el for el in xml_root.iter() if local(el.tag) == "CustomAction"}
    verbs = {
    ...
    for action_id, verb in verbs.items():
        ...
        attrs = action.attrib
        if (
            attrs.get("Directory") != "INSTALLDIR"
            or attrs.get("Execute") != "deferred"
            or attrs.get("Impersonate") != "no"
            or attrs.get("Return") != "check"
        ):
            fail(f"WiX custom action privilege/return contract drifted: {action_id}")
        command = attrs.get("ExeCommand", "")
        if "[INSTALLDIR]bin\\vsn-agent.exe" not in command or verb not in command:
            fail(f"WiX custom action command mismatch: {action_id}")

    sequence = {
        el.attrib.get("Action"): (el.attrib, (el.text or "").strip())
        for el in xml_root.iter() if local(el.tag) == "Custom"
    }
    required_sequence = {
    ...
    for action_id, (ordering, condition) in required_sequence.items():
        ...
        if any(attrs.get(key) != value for key, value in ordering.items()) or actual_condition != condition:
    ...
    require_tokens(
        harness,
        (
            "current-user", "per-machine", "msi", "VSN-Agent",
            "NT AUTHORITY\\LocalService", "--service-run",
            "function Invoke-Agent", "Invoke-Agent $agent stop $Lane",
            "Invoke-Agent $agent start $Lane", "function Invoke-Ping",
            "Invoke-Ping $cli $Lane", "tracked_repository_drift_zero",
        ),
        "lifecycle harness",
    )
    ...
    require_tokens(
        ...
        (
            ...
            "--bundles nsis", "--bundles msi", "pkg03-0311-agent-service-lifecycle",
        ),
        "workflow",
    )
