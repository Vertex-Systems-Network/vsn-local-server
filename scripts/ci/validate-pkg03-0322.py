...
CURRENT_BASE = "e3fb61581646a475c117cc893566286e397c2108"
...
def main() -> None:
    ...
    if (
        ...
        or current.get("done") != 21
        or current.get("required") != 25
        or current.get("percent") != 84.0
        or current.get("active_task") != "03.22"
        or current.get("ready_tasks") != ["03.22"]
    ):
        fail("current canonical package baseline is not accepted 21/25 state")
