...
./packaging/macos/build-pkg.sh 0.20.0 target/release /tmp/vsn019-macos-dist
pkgutil --check-signature /tmp/vsn019-macos-dist/vsn-runtime-0.20.0-unsigned.pkg >/dev/null 2>&1 || true
if [[ "${VSN_INSTALL_PACKAGE_SMOKE:-0}" == "1" ]]; then
  sudo installer -pkg /tmp/vsn019-macos-dist/vsn-runtime-0.20.0-unsigned.pkg -target /
...
echo 'VSN batch 0.20 macOS smoke PASS'
