...
Set-Location $Root
Write-Host '== VSN 0.9 Windows integration smoke =='
...
try {
  & $Cli status | Out-Host
  $Workspace = Join-Path $env:TEMP 'vsn-0.9-smoke'
  ...
  "echo VSN_PTY_WAIT_OK`r`nexit`r`n" | & $Cli terminal pty-write $pty.session_id | Out-Host
  $ptyOut = ((& $Cli terminal pty-read-wait $pty.session_id 3000) | Out-String) | ConvertFrom-Json
  if ($ptyOut.output -notmatch 'VSN_PTY_WAIT_OK') { throw 'PTY wait-read output missing' }
  ...
  Write-Host 'VSN 0.9 Windows integration smoke completed.'
}
finally {
