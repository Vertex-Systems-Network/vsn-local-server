#!/usr/bin/env python3
"""Static/offline validation for VSN 0.15 source artifacts.
...
try:
    ...
    if roadmap.get('product_version')!='0.15.0': errors.append('roadmap-status.json product_version must be 0.15.0')
    try:
...
# 0.15 integration anchors: static presence only, not compiler proof.
required={
 'crates/vsn-saml/src/lib.rs':['verify_acs_response','xmlsec1','idp_entity_id','duplicate XML ID','SAML assertion Issuer'],
 'crates/vsn-auth/src/lib.rs':['SamlProviderConfig','idp_entity_id','validate_saml','saml_providers'],
 'cloud/control-plane/src/main.rs':['0.15.0','saml_begin','saml_acs','link_saml_identity','scim_bulk','scim_reconcile','scim_require_if_match','store_passkey_owner'],
 'crates/vsn-database-cli/src/lib.rs':['QueryJobArtifact','QueryJobOutputChunk','read_query_job_output','MAX_JOB_ARTIFACT_BYTES'],
 'crates/vsn-terminal/src/lib.rs':['start_pty_session_with_scrollback','PtyScrollbackChunk','read_pty_scrollback','MAX_PTY_SCROLLBACK_BYTES'],
 'crates/vsn-update/src/lib.rs':['ApplyFileRequest','apply_verified_file','rollback_verified_file','confirm_apply'],
 'crates/vsn-core/src/lib.rs':['database_cli_job_output','terminal_pty_scrollback_read','update_apply_file','update_rollback_file'],
 'apps/agent/src/main.rs':['0.15.0','database.cli.job.output','terminal.pty.scrollback.read','update.apply-file','update.rollback-file'],
 'apps/cli/src/main.rs':['0.15.0','job-output','pty-scrollback-read','apply-file','rollback-file'],
 'contracts/saml-provider-v1.schema.json':['idp_entity_id','x509_certificate_pem_env'],
 'contracts/scim-bulk-v1.schema.json':['VSN SCIM 2.0 Bounded Bulk Request v1','Operations','failOnErrors'],
 'contracts/database-query-job-v1.schema.json':['artifact','preview_truncated','sha256'],
 'contracts/pty-scrollback-chunk-v1.schema.json':['VSN PTY Durable Scrollback Chunk v1','payload_base64'],
 'docs/saml-acs.md':['xmlsec1','idp_entity_id','explicitly link'],
 'docs/database-query-artifacts.md':['64 MiB','SHA-256','offset'],
 'docs/terminal-scrollback.md':['64 MiB','does not recreate a PTY'],
 'docs/updater-apply-rollback.md':['out-of-process','rollback','SHA-256'],
 '.github/workflows/release-gate.yml':['cargo clippy','cargo audit','validate-batch-0.15.py','SOURCE_SHA256SUMS.txt'],
 'scripts/release-gate.py':['0.15.0','SHA256','manifest'],
}
for rel,needles in required.items():
    p=ROOT/rel
    if not p.is_file(): errors.append(f'Missing 0.15 integration file: {rel}'); continue
    text=p.read_text(encoding='utf-8')
    for needle in needles:
        if needle not in text: errors.append(f'Missing 0.15 integration anchor {needle} in {rel}')
