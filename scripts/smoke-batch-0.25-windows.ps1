...
Set-Location $Root
Write-Host '== VSN 0.25 Windows integration smoke =='
...
try {
  ...
  if ($version -notmatch '0\.19\.0') { throw "unexpected CLI version: $version" }
  & $Cli status | Out-Host

  $Workspace = Join-Path $env:TEMP 'vsn-0.25-smoke'
  ...
  "echo VSN_PTY_019_OK`r`nexit`r`n" | & $Cli terminal pty-write $pty.session_id | Out-Host
  $ptyOut = ((& $Cli terminal pty-read-wait $pty.session_id 3000) | Out-String) | ConvertFrom-Json
  if ($ptyOut.output -notmatch 'VSN_PTY_019_OK') { throw 'PTY output missing' }
  ...
  if ($scrollText -notmatch 'VSN_PTY_019_OK') { throw 'durable PTY scrollback missing' }

  # Generic stream foundation smoke through authenticated Agent IPC.
  $stream = ('{"kind":"terminal","direction":"bidirectional","resource_id":"smoke-019","metadata":{}}' | & $Cli stream open | Out-String) | ConvertFrom-Json
  ...
  $install = Join-Path $env:TEMP 'vsn-0.25-update-smoke'
  ...
  $staged = Join-Path $env:TEMP 'vsn-0.25-staged.bin'
  ...
  $apply = @{ install_root=$install; target_relative='bin/demo.bin'; staged_artifact=$staged; expected_sha256=$sha; release='0.26.0-smoke'; confirm_apply=$true } | ConvertTo-Json -Compress
  ...
  if (Get-Command python -ErrorAction SilentlyContinue) {
    python scripts/validate-batch-0.25.py
    & .\target\release\vsn.exe commands | Out-Null
    & .\target\release\vsn.exe diagnostics | Out-Null
    python scripts/validate-schemas.py
  }

  if (Get-Command wix -ErrorAction SilentlyContinue) {
    cargo build -p vsn-agent -p vsn -p vsn-updater-helper --release
    & (Join-Path $Root 'packaging\windows\build-extension-appcontainer.ps1') -OutputDir (Join-Path $Root 'target\release')
    if (-not (Test-Path (Join-Path $Root 'target\release\vsn-extension-appcontainer.exe'))) { throw 'AppContainer helper missing after build' }
    & (Join-Path $Root 'packaging\windows\build-msi.ps1') -Version 0.26.0 -SourceDir (Join-Path $Root 'target\release') -OutputDir (Join-Path $env:TEMP 'vsn-0.25-msi-smoke')
  }
  ...
  Write-Host 'VSN 0.25 Windows integration smoke completed.'
}
finally {
