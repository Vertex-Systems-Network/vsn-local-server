...
./packaging/linux/build-deb.sh 0.22.0 target/release /tmp/vsn022-linux-dist
dpkg-deb -I /tmp/vsn022-linux-dist/vsn-runtime-0.22.0-amd64.deb >/dev/null
dpkg-deb -c /tmp/vsn022-linux-dist/vsn-runtime-0.22.0-amd64.deb | grep -q './usr/local/bin/vsn-agent'
if [[ "${VSN_INSTALL_PACKAGE_SMOKE:-0}" == "1" ]]; then
  sudo dpkg -i /tmp/vsn022-linux-dist/vsn-runtime-0.22.0-amd64.deb
  /usr/local/bin/vsn --version
  sudo dpkg -r vsn-runtime
fi
echo 'VSN batch 0.22 Linux smoke PASS'
