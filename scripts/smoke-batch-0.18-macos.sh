...
python3 scripts/validate-batch-0.18.py
...
rm -rf /tmp/vsn017-macos-dist; mkdir -p /tmp/vsn017-macos-dist
./packaging/macos/build-pkg.sh 0.18.0 target/release /tmp/vsn017-macos-dist
pkgutil --check-signature /tmp/vsn017-macos-dist/vsn-runtime-0.18.0-unsigned.pkg >/dev/null 2>&1 || true
if [[ "${VSN_INSTALL_PACKAGE_SMOKE:-0}" == "1" ]]; then
  sudo installer -pkg /tmp/vsn017-macos-dist/vsn-runtime-0.18.0-unsigned.pkg -target /
...
echo 'VSN batch 0.18 macOS smoke PASS'
