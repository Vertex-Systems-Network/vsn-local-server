#!/usr/bin/env python3
"""Static/offline validation for VSN 0.14 source artifacts.
...
try:
    ...
    if roadmap.get('product_version')!='0.14.0': errors.append('roadmap-status.json product_version must be 0.14.0')
    try:
...
# 0.14 integration anchors: static presence only, not compiler proof.
required={
 'crates/vsn-control-store/src/lib.rs':['vsn_control_roles','vsn_control_accounts','vsn_control_auth_transactions','vsn_control_auth_policy','vsn_control_scim_groups','SharedAuthPolicyRecord','upsert_auth_policy'],
 'crates/vsn-auth/src/lib.rs':['SamlProviderConfig','validate_saml','saml_providers','x509_certificate_pem_env'],
 'crates/vsn-database-cli/src/lib.rs':['QueryJobState','start_read_query_job','cancel_query_job','recover_interrupted_query_jobs','database query cancelled by operator'],
 'crates/vsn-core/src/lib.rs':['database_cli_job_start','database_cli_job_status','database_cli_jobs','database_cli_job_cancel'],
 'cloud/control-plane/src/main.rs':['0.14.0','backfill_shared_auth_once','refresh_shared_auth_state','sync_shared_auth_policy','scim_list_groups','scim_patch_user','scim_patch_group','SCIM_GROUP_SCHEMA'],
 'apps/agent/src/main.rs':['0.14.0','database.cli.job.start','database.cli.job.cancel','allow_remote_database_query'],
 'apps/cli/src/main.rs':['0.14.0','database.cli.job.start','database.cli.job.cancel'],
 'apps/desktop/src/App.tsx':['v0.14.0','function DatabaseJobs','database.cli.job.start','Cancel'],
 'cloud/dashboard/src/main.tsx':['database.cli.job.start','SAML trust configuration'],
 'contracts/database-query-job-v1.schema.json':['VSN Durable Database Read Query Job v1','interrupted','cancelled'],
 'contracts/scim-group-v1.schema.json':['VSN SCIM 2.0 Group Input','displayName','members'],
 'contracts/scim-patch-v1.schema.json':['VSN SCIM 2.0 PatchOp','Operations'],
 'contracts/saml-provider-v1.schema.json':['VSN SAML Provider Trust Policy v1','x509_certificate_pem_env'],
 'contracts/enterprise-auth-policy-v1.schema.json':['saml_providers','saml-provider-v1.schema.json'],
 'docs/database-query-jobs.md':['Interrupted','database.cli.job.cancel','exact child process'],
 'docs/saml-policy.md':['fail-closed','XML signature','x509'],
 'docs/scim-runbook.md':['Groups','PATCH','control.scim.manage'],
 '.github/workflows/release-gate.yml':['cargo clippy','cargo audit','validate-batch-0.14.py','SOURCE_SHA256SUMS.txt'],
 'scripts/release-gate.py':['0.14.0','SHA256','manifest'],
}
for rel,needles in required.items():
    p=ROOT/rel
    if not p.is_file(): errors.append(f'Missing 0.14 integration file: {rel}'); continue
    text=p.read_text(encoding='utf-8')
    for needle in needles:
        if needle not in text: errors.append(f'Missing 0.14 integration anchor {needle} in {rel}')
