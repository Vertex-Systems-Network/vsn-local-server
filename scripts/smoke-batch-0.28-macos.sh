...
rm -rf /tmp/vsn028-macos-dist; mkdir -p /tmp/vsn028-macos-dist
./packaging/macos/build-pkg.sh 0.29.0 target/release /tmp/vsn028-macos-dist
pkgutil --check-signature /tmp/vsn028-macos-dist/vsn-runtime-0.29.0-unsigned.pkg >/dev/null 2>&1 || true
if [[ "${VSN_INSTALL_PACKAGE_SMOKE:-0}" == "1" ]]; then
  sudo installer -pkg /tmp/vsn028-macos-dist/vsn-runtime-0.29.0-unsigned.pkg -target /
...
echo 'VSN batch 0.28 macOS smoke PASS'
