#!/usr/bin/env python3
"""Static/offline validation for VSN 0.18 source artifacts.
...
try:
    ...
    if roadmap.get('product_version')!='0.18.0': errors.append('roadmap-status.json product_version must be 0.18.0')
    try:
...
# 0.18 integration anchors: static presence only, not compiler proof.
required={
 'crates/vsn-vault/src/lib.rs':['VaultRotationResult','rotate_master_key','key_id','old_key_retained_for_recovery'],
 'crates/vsn-ai/src/lib.rs':['ExecuteRequest','ExecutionReport','unrestricted_shell_allowed'],
 'apps/agent/src/main.rs':['0.18.0','ai.execute','vault.rotate','marketplace.resolve-update','extension.providers','network.dns-start','dns-server'],
 'apps/cli/src/main.rs':['0.18.0','ai.execute','vault.rotate','marketplace.resolve-update','extension.providers','network.dns-start'],
 'crates/vsn-marketplace/src/lib.rs':['MarketplaceRevocation','resolve_update','is_revoked'],
 'crates/vsn-extension/src/lib.rs':['ResolvedProvider','resolve_providers','verify_manifest(&manifest,&runtime_trust)'],
 'crates/vsn-network/src/lib.rs':['DnsResolverPlan','run_dns_server','build_dns_response','127.0.0.1'],
 'crates/vsn-core/src/lib.rs':['vault_rotate','marketplace_resolve_update','extension_providers','dns_start'],
 'cloud/control-plane/src/main.rs':['0.18.0','/ready','/v1/admin/ops','async fn readiness','async fn ops_status','VSN_SLO_CONTROL_P95_MS'],
 'scripts/release-evidence.py':['vault-key-rotation','penetration-test','require-certified','completion_percent'],
 'docs/release-evidence-0.18.json':['vault-key-rotation','penetration-test'],
 ...
 '.github/workflows/release-gate.yml':['validate-batch-0.18.py','release-evidence.py','windows-msi','linux-deb','macos-pkg','cargo audit'],
 ...
 'scripts/smoke-batch-0.18-linux.sh':['cargo clippy','build-deb.sh','smoke-updater-helper.py'],
 'scripts/smoke-batch-0.18-macos.sh':['cargo clippy','build-pkg.sh','smoke-updater-helper.py'],
 'scripts/smoke-batch-0.18-windows.ps1':['cargo clippy','build-msi.ps1','pg-native-job-list'],
 'docs/roadmap-status.json':['0.18.0','overall_completion_percent','completion_percent'],
 'docs/BATCH-0.18.md':['Vault rotation','AI execution','release evidence','.test DNS'],
 'docs/P30-release-readiness-0.18.md':['21-control','release-evidence.py'],
}

for rel,needles in required.items():
    p=ROOT/rel
    if not p.is_file(): errors.append(f'Missing 0.18 integration file: {rel}'); continue
    text=p.read_text(encoding='utf-8')
    for needle in needles:
        if needle not in text: errors.append(f'Missing 0.18 integration anchor {needle} in {rel}')
