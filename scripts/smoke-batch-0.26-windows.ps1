...
Set-Location $Root
Write-Host '== VSN 0.26 Windows integration smoke =='
...
try {
  ...
  $Workspace = Join-Path $env:TEMP 'vsn-0.26-smoke'
  ...
  $install = Join-Path $env:TEMP 'vsn-0.26-update-smoke'
  ...
  $staged = Join-Path $env:TEMP 'vsn-0.26-staged.bin'
  ...
  if (Get-Command python -ErrorAction SilentlyContinue) {
    python scripts/validate-batch-0.26.py
  ...
  if (Get-Command wix -ErrorAction SilentlyContinue) {
    ...
    & (Join-Path $Root 'packaging\windows\build-msi.ps1') -Version 0.26.0 -SourceDir (Join-Path $Root 'target\release') -OutputDir (Join-Path $env:TEMP 'vsn-0.26-msi-smoke')
  }
  ...
  Write-Host 'VSN 0.26 Windows integration smoke completed.'
}
finally {
